...
test('builds the latest three PSMDB major images', () => {
  assert.ok(dockerBuildArgs('psmdb=6.0').includes('pmm-qa/psmdb:6.0'));
  assert.ok(dockerBuildArgs('psmdb=7.0').includes('pmm-qa/psmdb:7.0'));
...
test('builds Oracle Linux variants', () => {
  ...
  assert.ok(args.includes('pmm-qa/psmdb:8.0-ol8'));
  assert.equal(parseConfig([], { OL_VERSION: '8' }).image, 'pmm-qa/psmdb:8.0-ol8');
});
...
test('uses PSMDB 8 wiredTiger PSS defaults', () => {
  ...
  assert.equal(config.image, 'pmm-qa/psmdb:8.0');
...
test('builds and parses MongoDB Community', () => {
  assert.ok(dockerBuildArgs('mongodb=6.0').includes('pmm-qa/mongodb:6.0'));
  const config = parseConfig(['--engine', 'mongodb', '--version', '8.0'], {});
  assert.equal(config.engine, 'mongodb');
  assert.equal(config.image, 'pmm-qa/mongodb:8.0');
