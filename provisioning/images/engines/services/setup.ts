...
const LABEL = 'pmm-qa.engine=services';
...
export function parseConfig(argv: string[] = process.argv.slice(2), env = process.env): Config {
  const { values } = parseArgs({ args: argv, strict: true, options: {
    type: { type: 'string' }, image: { type: 'string' }, ...PMM_CLIENT_OPTIONS,
  ...
  return {
    type, image: values.image ?? env.SERVICE_IMAGE ?? `pmm-qa/${type}:latest`,
    redisImage: values['redis-image'] ?? env.REDIS_IMAGE ?? 'valkey/valkey:8-bookworm',
