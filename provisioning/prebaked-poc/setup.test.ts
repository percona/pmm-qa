...
test('builds the selected MySQL image', () => {
  ...
  assert.ok(args57.includes('prebaked-57'));
  assert.ok(args57.includes('pmm-qa/mysql:5.7-prebaked'));
  assert.ok(args57.includes('engines/mysql/Dockerfile'));

  const args97 = dockerBuildArgs('mysql=9.7');
  assert.ok(args97.includes('prebaked-epel'));
...
test('builds the selected PS image with its matching XtraBackup', () => {
  ...
  assert.ok(args.includes('pmm-qa/ps:5.7-prebaked'));
...
test('accepts engine defaults and engine=version overrides', () => {
  assert.ok(dockerBuildArgs('ps=8.4').includes('pmm-qa/ps:8.4-prebaked'));
  assert.ok(dockerBuildArgs('psmdb=8.0').includes('pmm-qa/psmdb:8.0-prebaked'));
...
test('builds PSMDB and PDPGSQL from their engine directories', () => {
  ...
  assert.ok(psmdb.includes('pmm-qa/psmdb:8.0-prebaked'));

  const pdpgsql = dockerBuildArgs('pdpgsql=18');
  assert.ok(pdpgsql.includes('engines/pdpgsql/Dockerfile'));
  assert.ok(pdpgsql.includes('pmm-qa/pdpgsql:18-prebaked'));
...
test('parses MySQL single-node defaults', () => {
  ...
  assert.equal(config.image, 'pmm-qa/mysql:8.4-prebaked');
...
test('parses PS single-node defaults', () => {
  ...
  assert.equal(config.version, '8.4');
  assert.equal(config.image, 'pmm-qa/ps:8.4-prebaked');
});
...
test('selects the image and replication dialect by MySQL version', () => {
  ...
  assert.equal(legacy.image, 'pmm-qa/mysql:5.7-prebaked');
  ...
  assert.equal(parseConfig(['--engine', 'mysql', '--version', '9.7'], {}).image, 'pmm-qa/mysql:9.7-prebaked');
...
test('selects the image and replication dialect by PS version', () => {
  ...
  assert.equal(legacy.image, 'pmm-qa/ps:5.7-prebaked');
