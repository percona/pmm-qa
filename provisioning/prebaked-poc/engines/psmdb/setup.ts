...
const LABEL = 'pmm-qa.poc=psmdb-prebaked';
const MINIO_CONTAINER = 'psmdb-poc-minio';
const MINIO_VOLUME = 'psmdb-poc-minio-backups';
const KERBEROS = 'kerberos';
const KEYTABS_VOLUME = 'psmdb-poc-keytabs';
...
function topologyPrefix(setupType: SetupType): string {
  return setupType === 'pss' ? '' : `${setupType}_`;
}
...
export function parseConfig(
  ...
  const { clientVersion, clientTarball } = selectClientSource(
    values['client-version'] ?? env.PMM_CLIENT_VERSION ?? env.CLIENT_VERSION,
  ...
  return {
    ...
    image: values.image ?? env.PSMDB_IMAGE ?? env.MONGODB_IMAGE
      ?? `pmm-qa/${engine}:${version}${engine === 'psmdb' && env.OL_VERSION ? `-ol${env.OL_VERSION}` : ''}-prebaked`,
    setupType,
    storageEngine,
    clientVersion,
    clientTarball,
    pmmServer:
      values['pmm-server'] ?? env.PMM_SERVER_CONTAINER_ADDRESS ?? env.PMM_SERVER_IP,
...
export function topology(setupType: SetupType): MongoNode[] {
  const prefix = topologyPrefix(setupType);
...
async function startKerberos(config: Config): Promise<void> {
  ...
  await requireDockerImage('pmm-qa/kerberos:prebaked', 'npm run build -- psmdb');
  ...
  await docker(['run', '--detach', '--name', KERBEROS, '--hostname', KERBEROS,
    ...
    '--env', `MONGO_HOSTS=${hosts}`, 'pmm-qa/kerberos:prebaked']);
...
async function registerWithPmm(config: Config, nodes: MongoNode[]): Promise<void> {
  ...
  await Promise.all(
    nodes.map((node) =>
      retry(
        `PMM MongoDB exporter on ${node.name}`,
        () => docker(['exec', node.name, 'pmm-admin', 'status'], true),
        (result) =>
          /mongodb_exporter/i.test(result.stdout) && /(running|waiting)/i.test(result.stdout),
      ),
    ),
  );
}
