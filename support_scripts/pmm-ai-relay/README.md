...
| PR Maintainer digest → Slack | The daily `pr-maintainer` Routine uses the shared [`relay` skill](https://github.com/percona/pmm-ai/blob/main/plugins/pmm-qa/skills/relay/SKILL.md) to send `{channel,text}` through `POST /slack/announce`; the relay posts it as the bot to `#qa-automation`. Invite the bot to that channel first, and set `RELAY_KEY` in `.env`. `RELAY_KEY` is the shared-env → relay bearer, reused by every broker endpoint. |
| Zephyr test-case creation | The [`test-cases` skill](https://github.com/percona/pmm-ai/blob/main/plugins/pmm-qa/skills/test-cases/SKILL.md) uses the shared [`relay` skill](https://github.com/percona/pmm-ai/blob/main/plugins/pmm-qa/skills/relay/SKILL.md) to reach `POST /zephyr/<action>` (`search`, `create`, `get`, `list`, `statuses`, `folders`, `set-status`, `steps`, `link-issue`, `create-folder`), so a new automated test can be born with its own `PMM-Txxxx`. Needs **`ZEPHYR_PMM_API_KEY`** in the relay `.env` — a Zephyr Scale API key for the `PMM` project, the same value CI already holds as a GitHub secret. Without it the broker answers `503 zephyr_not_configured` and nothing else is affected. Mirror it in the LastPass **PMM** `.env` note. |
