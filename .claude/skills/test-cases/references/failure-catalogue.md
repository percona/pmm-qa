...
One entry per mechanism: how the product could be wrong, how to probe it, and the PMM bugs where it happened. Read it after tracing the path in `change-impact-and-failure-model.md`, and write every mechanism the traced path can reach into the notes as a hypothesis. A `PMM-*` key is a Jira bug; a `PMM-T*` key is a Zephyr observation. History is evidence only when its mechanism is reachable from the current change; a shared component or page is not.
...
- **Denial looks empty:** an authorization or privilege failure renders like no data. Assert the API result and distinguish forbidden from empty. History: PMM-15206, PMM-15336, PMM-14661, PMM-14717.
...
- **Rejected input persists:** validation returns an error but stores the value, or a removal on one surface leaves another surface broken. Read the source of truth after rejection; assert the exporter stays Running. History: PMM-15130, PMM-15201, PMM-13120, PMM-15242.

## Propagation between layers

- **Stored but not consumed:** UI or API shows the value while the exporter, metric, or agent uses the old one. Assert storage and the final consumer after the relevant reload. History: PMM-15074, PMM-15354, PMM-15241, PMM-14751, PMM-14931.
...
- **Auth or TLS variant breaks collection:** a new auth default, skip-verify, or certificate path works only for the variant the developer tried. Exercise the variant and DB version the change touches. History: PMM-15259, PMM-14519, PMM-15188, PMM-7888.
...
- **Cluster state lies:** stale inventory, false health, wrong proxy routing, or restart failure in HA. Drive the real cluster trigger, assert at the owning layer, and test unhealthy as well as healthy. History: PMM-15227, PMM-14734, PMM-15029, PMM-15228.
- **Chart differs from single-server:** defaults, image pins, or feature gates change what exists. Render effective values before assuming a component is present. History: PMM-T2217, internal PostgreSQL QAN disabled by chart values.
- **Restore leaves the database down:** the restore reports success, tool versions conflict, or polling misreports state. Perform a real restore and assert the DB is running and queryable. History: PMM-15163, PMM-14594, PMM-14576.
...
- **Old code drives the upgrade:** old prechecks evaluate a new artifact with obsolete assumptions. Exercise the precheck from the old side.
- **Recovery precedes readiness:** startup replays persisted state before dependencies are ready and loops. Seed several stale items and prove a new request succeeds. History: PMM-15404, PMM-15266, PMM-15050.
...
- **Works fresh, not upgraded:** a datasource, plugin, theme, or dashboard is correct on a fresh install and broken after upgrade with seeded state. History: PMM-15155, PMM-15083.
...
- **UI hides, API allows:** the action is hidden while the backend accepts the mutation. Deny at the API boundary and verify unchanged state. History: PMM-15138, PMM-15139, PMM-15309, PMM-15067.
...
- **Secret in logs or responses:** bound the exact window and search for the exact test secret. History: PMM-15164, PMM-15010.
...
- **Filter omits a topology:** All or multi-value filters drop standalone or asynchronous nodes. Exercise the non-default topology with several values selected. History: PMM-15240, PMM-13932, PMM-14908, PMM-14938.
