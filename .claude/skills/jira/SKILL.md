...
# Response body is {"issues": [...], "total": N} — read .issues[] and .total, not .body.issues[].
# A wrong jq path and a clean dedup both print nothing, so check the raw payload before trusting zero rows.
