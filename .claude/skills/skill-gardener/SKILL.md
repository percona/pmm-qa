...
The repository injects a two-sentence observer reminder through `UserPromptSubmit` for the main agent and `SubagentStart` for every subagent, and re-injects it through `PostToolUseFailure` when a tool call fails. The turn-start reminders prime observation; the failure one is the only point where the harness signals that lesson evidence has actually just been produced, so it carries most of the capture. This keeps observation inside the model calls already needed for the task instead of forcing another LLM pass after every response. `Stop` is deliberately not used: it has no `additionalContext` channel, so reaching the model from it means `decision: "block"`, which forces exactly the extra stop-time pass this design avoids. Load this full skill only when a possible lesson appears or the user invokes it directly. Set `SKILL_GARDENER=off` to silence the reminder for a session.
