...
6. Create one immutable file under `.claude/skill-lessons/` named `<date>-<full-session-id>-<agent-id-or-main>-<nn>.md`, where `<nn>` is the next two-digit ordinal not already used by that prefix. Resolve that ordinal against the **week branch**, not the checkout — `git ls-tree --name-only origin/skill-gardener/<YYYY>-W<WW> .claude/skill-lessons/ | grep <prefix>`, falling back to the checkout only when that ref is unavailable — because step 4 removes pushed entries from the checkout, so a later writer in the same session sees an empty directory and reuses names already committed. Write with a no-clobber mechanism (`set -o noclobber`, or `cp -n`) so a collision fails loudly: a plain `>` redirection silently overwrote four immutable entries an earlier capture had committed, visible only as ` M` in `git status`. Normalize ID components to lowercase filesystem-safe text. If that exact observation already exists, do not duplicate it. Session and agent IDs keep concurrently captured related evidence distinct without locks; the ordinal only separates repeat captures by the same agent.
...
2. Copy the turn's new entries into the worktree, commit only those files, and push. Address every git and file operation absolutely — `git -C "$WT" …`, `"$WT/.claude/skill-lessons/<file>"` — and assert the worktree exists (`test -d "$WT/.claude/skill-lessons" || exit 1`) before writing anything. A bare `cd "$WT/..."` that fails leaves execution in the user's shared checkout, where the entry lands in the repo root and the commit goes onto whatever branch another agent is using. A name that already exists on the branch with differing content is a **renumber**, never a replace.
