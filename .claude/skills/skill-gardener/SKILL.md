---
name: skill-gardener
description: Continuously audit every observable main-agent and subagent turn for reusable workflow lessons without forcing an extra stop-time LLM pass. Capture conflict-resistant lesson entries onto the open shared gardener branch and publish them in its single lessons PR, then apply high-confidence improvements to skills, agents, hooks, or shared instructions from the entries already merged into main. Also use when asked to capture or review lessons, improve a skill from experience, or identify a new skill candidate. Do not treat hidden reasoning, routine task facts, or one-off preferences as lessons.
---
...
- **Capture:** Preserve each distinct qualifying lesson as an immutable entry on the open shared gardener branch, published immediately in that batch's single lessons-only PR.
...
3. Search `.claude/skill-lessons/` in the checkout, on the open batch branch, and on `origin/main` for related open evidence before writing, so the entry can identify the same lesson. An entry on `main` is one Publish has not resolved yet. Related evidence is still useful recurrence; skip only an exact duplicate observation.
...
1. Fetch `origin` and find the open lessons batch: the `skill-gardener/*` branch that still has an open lessons PR. Create an isolated temporary worktree on that branch if one exists, so every lesson caught since the last merge accumulates into the same PR. Only when no batch is open — the last one merged, or this is the first ever — create `skill-gardener/<YYYY-MM-DD>` for the current date from the latest `origin/main`. Never switch, reset, stash, or alter unrelated files in the user's checkout.
2. Copy the turn's new entries into the worktree, commit only those files, and push.
3. Never rewrite or force-push a commit already pushed to the shared batch branch. Other sessions push to it too, and a force-push silently drops their commits. If your push is rejected, run `git pull --rebase` in the worktree — it replays only your own unpushed commits on top of theirs, rewriting nothing they can see — then push again; unique entry filenames keep that conflict-free.
...
5. Publish the lesson immediately: if you pushed to an already-open batch, stop — that PR picks up your commit on its own. If you started a new batch branch, open its lessons-only PR against `main` now. Either way exactly one lessons PR is open at a time, holding every lesson caught since the last merge, so lessons never accumulate unpublished and never fan out into a PR each. Never reopen a closed PR; if a batch's PR was closed deliberately, start a new batch rather than reviving it.
...
- Never edit a target outside Publish. A session opens only the batch's lessons-only PR; Publish opens one implementation PR per run. Exactly one lessons PR is open at a time, never one per lesson.
