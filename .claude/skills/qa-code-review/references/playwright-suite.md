...
| A ported assertion's strength is set by its CodeceptJS source — matcher **and** guards | stricter than the source is a coverage change needing evidence, not a tightening. `dontSeeElement` passes on a hidden element, so `filter({ visible: true })` before `toHaveCount(0)` is the faithful port; likewise an assertion the source makes only inside AND-ed guards stays inside them |
| A migrated DataTable title keeps its literal suffix | `JSON.stringify(row)` cannot reproduce it: `perfectionist/sort-objects` alphabetises the object literal while CodeceptJS keys it in `new DataTable([...])` column order, and a renamed POM key diverges too. Deriving the suffix from the row is right for a native test and wrong for a ported one |
| `collectTextsAcrossScroll` dedupes by text | never recommend it over a raw `allTextContents()` where the count feeds a threshold: its `Set` drops duplicate panel titles and shrinks the count toward the budget it protects. After `loadAllPanels()` every grid item stays mounted, so the raw call does not undercount |
...
- Cleanup in `afterEach`, not `try/finally`. Shared setup in `beforeAll` when it is genuinely once. Cleanup **asserts its own success** — restore, then read the restored state back (`getDataSourceByName()` after a password restore) — so a failed restore fails this test instead of leaking into the next.
