...
| A migrated DataTable title keeps its literal suffix | `JSON.stringify(row)` cannot reproduce it — `perfectionist/sort-objects` alphabetises the migrated object literal while CodeceptJS keys the JSON in `new DataTable([...])` column order, and a renamed destination POM key diverges too (7 of 8 titles measured wrong). Deriving the suffix from the row is right for a native test and wrong for a ported one |
