...
- **A claim that a tag is new or unused is made against its token-order permutations, not the literal string.** CodeceptJS orders these DB-first (`@pgsm-pmm-integration`, `@pdpgsql-pmm-integration`) and Playwright orders them pmm-first (`@pmm-psmdb-integration`), so "`@pmm-pdpgsql-integration` exists nowhere in the repo" was wrong by one word swap — `main` already carried the same database under the other order. A near-identical existing tag is itself the finding, because `runner-e2e-tests-playwright.yml:220` runs `--grep "$PMM_TEST_FLAG" … || true` and a flag matching nothing runs zero tests and reports green.
- **Before faulting a retry loop for not exiting on exhaustion, read the step after it.** The image-pull loops in the codeceptjs and playwright runners fall through deliberately: the compose files fetch nine images and one token failure aborts the whole `pull` even when eight are local, so the following `docker compose up -d` re-attempts only what is missing and exits non-zero naming the image it could not get. Where the next step re-attempts the same work on a narrower input and fails loudly, the fallthrough is the design and the finding does not stand.
