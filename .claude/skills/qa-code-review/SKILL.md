...
- No lint or type check covers what the diff touches → say so plainly in the summary, review the rest statically, and never substitute your own run for a missing gate. Answer the coverage question from the selectors in `.claude/hooks/lint-changed.sh` — `lint.yml` pipes every tracked file into it, so grepping the workflows for a linter's name proves nothing — and name the selector or gate that excludes the files: the eslint group's `'\.ts$'`, the `grep -q '"lint"' package.json` workspace gate, or a file kind with no group, markdown among them. `codeceptjs-e2e` exposes only `lint:tests` and so has no CI lint gate; say so.
...
**`.github/scripts/**`** is CI glue: `#!/usr/bin/env bash`, `set -euo pipefail`, a Usage/Environment header where the script takes arguments, shellcheck-clean, every external command time-bounded, and an exit code that cannot fail the calling step by accident.

**`.claude/**`** is instruction text; read it for design defects, not consistency: a dependency on another skill or agent the file does not need, repo/schedule/Routine specifics baked into reusable text, a "what not to do" phrasing where the positive instruction does the same job, and text that changes no behaviour.
...
4. **Reachability.** A new or retagged test is reachable from a workflow in the same PR. Prefer an existing tag and runner; inventing a tag, or widening a workflow's `--grep` to rescue a test that is missing its file's own tag, is 🔴. Before calling a test unreachable or its config missing, rule out the `Percona-Lab/jenkins-pipelines` entry point — suites such as `@pmm-ha` run from a pipeline that binds credentials in its own `environment {}` block and never dispatches this repo's workflows — or flag the claim as unverified; a grep of `.github/workflows/` alone never establishes the negative.
...
7. **Over-engineering.** Hand-rolled machinery is a finding until proven necessary. Duplicated blocks, a helper or const file with one caller, two tests where one covers the regression, a new tag where an existing one fits — all 🟡 or worse. When the remedy is to **delete** a CI job, a workflow entry or a test, state the duplication and its cost and ask the author to confirm — never prescribe the deletion. Run cost against dedicated coverage is the author's trade-off: one such thread got a job deleted and pushed, then reversed by the author who wanted it kept, costing two pushes and a correction.
   ...
   Two limits. Where the hand-rolled block is a **flake fix with a measured mechanism**, a "use the existing helper" substitution that changes the measured interaction (`I.click` for `I.appendField(field, '')` on an editor known to under-clear) is 🔵 pending a run on the affected widget, not 🟡; the duplication half of the finding stands on its own. "It is a port" does **not** defer a duplication finding: a behaviour-preserving migration constrains execution order and state handoff, not code shape.

   Where the remedy is to **drop an explicit value and inherit a config default**, name the inherited value and its effect on the failure budget, not just the lines saved: `waitForElement` without its second argument inherits `waitForTimeout: 60000` from `codeceptjs-e2e/pr.codecept.js:26`, and on a flake fix the explicit const carries the measured floor.
8. **New `eslint-disable`.** The `--` reason must state a real invariant. `TODO` as the reason means the code is not ready.
9. **Bot findings.** CodeRabbit findings are claims to verify, not noise: an unanswered valid one is 🟡. Never restate a finding that already has a thread. Read the PR's existing threads — **resolved and outdated ones included** — and the decisions stated in the body before writing anything, and sweep every bot — CodeRabbit, Copilot, any other — for a finding the author never answered or resolved. Each one that is still open and still valid gets a thread asking for it to be answered and resolved; each one that is open but wrong gets a thread saying so, so the author can close it with a reason instead of leaving it hanging. A finding that already has a thread gets a **reply inside that thread**, never a parallel one. A finding with no file line — a PR-level bot verdict, a problem with the body or the scope — goes in the summary comment. A finding that would **reverse a decision the PR already recorded** — in the body or in a settled thread — takes check 7's confirm-don't-prescribe form: state the cost and ask the maintainer, never a 🟡 with a suggestion block.

   Raise CWE-295 on a **skip-verification** call (`tls_skip_verify: true`, `curl --insecure`) only for a real inconsistency — one call verifying, its sibling not. Against test infrastructure with a self-signed cert (PMM Server's dev cert, a QA database) it is expected where the rest of the file or job already skips, and enabling it in one spot fails the call without hardening anything.
...
- Anchor from the **head**, not from diff arithmetic: re-resolve the head SHA immediately before posting, pin the pending review's `commitID` to it, and take line numbers from `git fetch origin pull/<number>/head:pr-head` plus `git show <sha>:<path> | cat -n` — `origin/<head-branch>` does not exist when the PR comes from a fork. A head that advanced by one commit mid-review mis-anchors every suggestion counted from `get_diff` hunks. When the re-resolved head differs from the SHA the findings came from, **re-read the changed files at the head and drop or rewrite whatever it already fixed** — re-anchoring alone leaves a stale finding, and a stale suggestion block, on correct coordinates.
