...
- Anchor from the **head**, not from diff arithmetic: re-resolve the head SHA immediately before posting, pin the pending review's `commitID` to it, and take line numbers from `git fetch origin pull/<number>/head:pr-head` plus `git show <sha>:<path> | cat -n` — `origin/<head-branch>` does not exist when the PR comes from a fork. A head that advanced by one commit mid-review mis-anchors every suggestion counted from `get_diff` hunks.
- End every thread and summary you post with the Claude Code attribution footer — a `---` rule, then `_Generated by [Claude Code](https://claude.ai/code)_`. `claude-code-review.yml` posts as `claude[bot]` and is recognisable by its login, but a contributor running this skill locally posts under their own account, and section 7 test 2 is the only thing that tells those threads from a human's.
...
## 7. Human comments are not Claude's

A comment is a human's only when all three hold:

1. the author login does not end in `[bot]` — that alone excludes `claude[bot]`, `coderabbitai[bot]`, `copilot-pull-request-reviewer[bot]` and `github-actions[bot]`;
2. the body carries no markdown link labelled `Claude Code` pointing at `claude.ai` or `claude.com`. The login proves nothing on its own: a Claude Code session posts under its operator's own account, and the footer has two live wordings (`_Generated by …_`, `_🤖 Addressed by …_`);
3. the body says something past a bare ` ```suggestion ` block. Five such empty blocks on #1366 are an author applying suggestions, not feedback.

The three fail safely in one direction only: a Claude comment misread as human costs a lesson candidate the gardener's Publish pass then rejects, while a human comment misread as Claude is lost silently. Where a comment is ambiguous, treat it as a human's.

Read those before writing anything, alongside the bot sweep section 3.9 asks for. Two rules follow from them:

- **Never re-raise a finding a human rejected.** Drop the rule for this diff, or say in one sentence why the rejection does not hold — never re-post the original thread.
- **A human finding on a line an earlier review passed is a gap in these rules, not a thread to duplicate.** Leave the human's thread alone.

A review captures no lesson itself — `claude-code-review.yml` runs with `contents: read` and cannot push. `.claude/agents/review-feedback-gardener.md` sweeps these comments daily and hands them to the skill gardener.
...
- Never re-raise a finding a human rejected (section 7).
