...
- Thread body: **one paragraph, then a ` ```suggestion ` block** carrying the concrete change, anchored on the exact lines it replaces (an empty block deletes them). Lead with the emoji. Prose describing a fix that a suggestion could have applied is the failure mode to avoid; a fix spanning other files or regions gets companion threads naming the main one.
