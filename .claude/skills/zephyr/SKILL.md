...
`traceability`, `raw`, `passthrough` — and the Zephyr API key is not in this environment,
so the upstream endpoint (which takes the Jira issue's **numeric id**, not its key) cannot
be reached either. Say so *before* creating the cases, not after: one run created six and
