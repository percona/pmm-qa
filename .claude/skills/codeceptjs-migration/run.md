...
3. on control's own checkout (never switched away), updates the tracker row to `done` with the PR link and pre-migration graph-refresh result, then commits and pushes only the tracker change. Edit the row as an anchored substring replacement, never a whole-file rewrite: `tracker.md` is LF-only and a whole-file write flips every line to CRLF here. Check `git diff --numstat` shows `1 1` and `grep -c $''` returns 0 before staging; and
