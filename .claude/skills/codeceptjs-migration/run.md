...
- Grep every migrated tag in `Percona-Lab/jenkins-pipelines` `pmm/v3/*.groovy` as well (`branch-workflow.md` Workflow coverage, section 1 command), whether or not a `.github` job selects it. A tag Jenkins passes to a CodeceptJS-only pipeline needs a Playwright step in that pipeline: name it in the report, and the PR stays unmergeable until the jenkins-pipelines PR adding it is open and linked.
