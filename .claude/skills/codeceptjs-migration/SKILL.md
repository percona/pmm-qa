...
- Pin every `.retry(N)` exactly: `Feature.retry` becomes file-scope `pmmTest.describe.configure({ retries: N })`; scenario-level becomes the same when all agree, else file-scope N for the majority and an anonymous `pmmTest.describe(() => { pmmTest.describe.configure({ retries: M }); ... })` around each exception, which adds no title prefix. The config default is `CI ? 2 : 0`, so no N survives unpinned. A titled wrapper prefixes the title: re-run the selectability check (`run.md` step 8). When appending to an existing file, wrap only the appended tests; the file's existing tests keep their pin or the default. Record N and scope in the commit message.
