...
- **A dashboard gets its own page object, and that is not a one-consumer file.** One `DashboardInterface` class per dashboard under `pages/dashboards/<family>/` with `url`, `metrics: GrafanaPanel[]` written as an inline literal (never a `const` list mapped into it), `noDataMetrics`, registered on `Dashboards` and its family index. Panel names live there, not in the test. The test iterates `dashboard.<x>.metrics` with one `pmmTest` per panel and reuses `verifyMetricsPresent`, `verifyPanelValues`, `verifyAllPanelsHaveData` where they fit. `pages/dashboards/valkey/` is the precedent.
...
When a human reviewer names a concrete end state, ship it in this PR; do not offer a follow-up or ask which variant. Push back only when the request breaks an invariant above, and name it. Answer every open bot thread either way. A bot predicting a locator or strict-mode failure is answered with the rendered-DOM count through MCP and the green strict-mode run that would have thrown on exactly that condition, plus the mechanism behind its evidence (dashboard JSON counts panels in collapsed rows that never render); that closes the thread, prose does not. A finding the bot withdrew gets no code change: adding panel-id scoping after proving the duplicates never render is a second review round for nothing. This repo squash-merges commit messages, not the PR body, into `main`: verify every attribution before it enters a commit body, and keep a hedged finding hedged.
