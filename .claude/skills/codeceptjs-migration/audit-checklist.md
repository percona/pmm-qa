...
- [ ] Title fidelity was proved by extracting both lists programmatically and printing them as quoted strings side by side - not by reading the two files, which hides a trailing space. Commented-out and skipped scenarios appear in that output too.
- [ ] For a data-driven scenario, expected titles came from the real runner: `npx codeceptjs dry-run -c pr.codecept.js --grep '<tag>'` from `codeceptjs-e2e/`. That also proves the source still parses and is still discovered.
