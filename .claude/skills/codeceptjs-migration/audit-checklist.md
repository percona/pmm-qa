...
- [ ] Every migrated assertion was checked for whether it can fail at the point it is evaluated, and the one most at risk was proven able to fail by mutating its expected value and re-running. Inverting a matcher is not a mutation.
...
- [ ] No newly added POM or helper method has zero callers. A single caller is not a finding on its own: about a third of existing page-object methods have exactly one, and `e2e_tests/eslint.config.mjs` whitelists single-caller assertion helpers by name. Raise a one-caller method only when something else is wrong with it - it wraps a fixed pause, or a web-first matcher already does the job. New surfaces only - exposing an existing method for reuse is `SKILL.md` Minimal reuse diffs and is not a finding.
