...
- [ ] Scenario titles and every original tag are preserved byte-identically at this gate. A destination execution tag may be added; it never replaces a source tag. A reviewer may approve a clearer title only *after* both gates pass (`SKILL.md` Migration invariants), but every tag and the `PMM-Txxxx` token survive any retitle verbatim - `--grep` matches tags inside the title, and `fixtures/pmmTest.ts` parses the id to drive the version gate.
- [ ] Title fidelity was proved by extracting both lists programmatically and printing them as quoted strings side by side - not by reading the two files, which hides a trailing space. Take the Playwright list from the **`list` reporter**: the `json` reporter does not preserve declaration order - it emits file-level specs before describe-block specs - so a file with a `describe` wrapper (any retry-pinned file) comes back rotated and an index-aligned diff reports every row as differing while the multiset is identical. Use `json` only for a multiset check. Commented-out and skipped scenarios appear in that output too.
...
- [ ] The data-row suffix was **not** built with `JSON.stringify(row)`. CodeceptJS emits `${title} | ${dataRow.data}` (`replaceTitle` in `codeceptjs/lib/data/context.js`) keyed in the **column order** passed to `new DataTable([...])`, while `perfectionist/sort-objects` alphabetises the migrated literal - a stringify-based target diverges silently. Reconstructing through `DataTable`/`Data()` corroborates the runner output; it is never the only check, and never a hand-typed expectation.
