...
Capture each selection to a sorted file and report added and removed sets with `comm -13`/`comm -23`, never two totals. Normalise coordinates out first, `sed -E 's/:[0-9]+:[0-9]+ / /'`; Playwright's separator is `›`, not `>`. Do not parse `Total: N tests` (misses `Total: 1 test`) and do not count with `git grep <rev> -- 'dir/**/*_test.js'` (`**` does not span directories); measure each count twice by different mechanisms.
