...
`PMM_UI_INSECURE=1` is in fact what works on the **single-server Docker path too**, from a session behind the egress proxy: measured on a provisioned run, the pin failed at `net::ERR_CERT_AUTHORITY_INVALID` and `openssl s_client` showed the host serving `issuer=O = Anthropic, CN = Egress Gateway SDS Issuing CA (production)` — the gateway's cert, not PMM's. Reach for `PMM_CERT_PATH` on a direct, unproxied path; reach for `PMM_UI_INSECURE=1` when the pin fails.
