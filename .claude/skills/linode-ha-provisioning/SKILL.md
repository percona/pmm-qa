...
if jq -e .kubeconfig_b64 "$RUN_DIR/provision.json" >/dev/null 2>&1; then
  ...
  kubectl get nodes -o json | jq '[.items[] | {name: .metadata.name,
    ...
    allocatable: .status.allocatable}]'                            >"$RUN_DIR/nodes-capacity.json"
...
helm upgrade --install pmm-ha /tmp/phc/charts/pmm-ha -n pmm --reset-then-reuse-values \
...
kubectl exec -n pmm statefulset/pmm-ha -- bash -c 'PGPASSWORD="$GF_DATABASE_PASSWORD" psql \
  -h "$GF_DATABASE_HOST" -U "$GF_DATABASE_USER" -d "$GF_DATABASE_NAME" \
  -c "DELETE FROM login_attempt;"'
```

On a genuine 401, reset Grafana's admin inside a pmm pod:
...
The sweeps above are the pattern; a new script that lists, creates or deletes Linode resources follows the same four rules, each one paid for by a real incident:

- **Key a destructive sweep on a tag unique to what the script created.** `pmm-qa-ephemeral` is stamped by `terraform/linode-runner/main.tf` and `create-lke-pmm-ha.sh` too, so a teardown defaulting to it would have deleted other people's live QA instances mid-test. Match the tag as an **exact array element** (`--json | jq --arg tag "$TAG" '.[] | select((.tags // []) | index($tag))'`) — `grep -qw` treats `-` as a word boundary, so `pmm-qa-perf` matches `pmm-qa-perf-run:<id>` as well. Default to `--dry-run` or a per-run tag, and never claim a sweep "deletes nothing else" when its key tag has other producers.
- **Page every account-wide listing explicitly** (`--page-size 500`, loop until a short page). `linode-cli linodes list` returns one 100-row page by default: a destructive teardown then reports `failed=0` while instances keep billing, and an inventory silently runs against fewer hosts than were provisioned. `prune-tags.sh`, `prune-lke-orphans.sh` and `relay.js` all already page.
