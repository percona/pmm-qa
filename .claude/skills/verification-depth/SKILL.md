...
In HA, `${PMM_URL}/prometheus/...` is not routed (it answers `missing route`); query VictoriaMetrics through the Grafana datasource proxy instead — `${PMM_URL}/graph/api/datasources/proxy/uid/<uid>/api/v1/query_range`, taking `<uid>` from the `prometheus`-type entry in `${PMM_URL}/graph/api/datasources`. The query parameters and the sample-count bar are unchanged.
