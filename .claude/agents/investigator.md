...
3. **Classify from what you observed** — one decision tree, not two:
   ...
   - **Reproduced, and current behavior actually matches what's intended** (confirmed by reading the relevant code, not just observing the UI/CLI) → **not a bug**. If someone's waiting for an answer — a question or suspected-bug ask — explain the correct way to do what they were after, grounded in what you just reproduced and the code you read, and stop; no fix. If this came from a **known CI/FB failure** instead, there's nobody to explain anything to — it means the *test's own expectation* is what's stale, not the product; continue as a **test bug** below, since the test still needs correcting even though nothing in PMM itself is actually broken.
   - **Reproduced, and it's a genuine bug** → decide where it actually lives:
     - **Product bug** (PMM/Grafana's own behavior contradicts what's intended — cross-check with `git-diff` against what merged upstream in the relevant window as *supporting* evidence, not the sole basis) → do **not** attempt a pmm-qa fix. Report it (comment linking the suspected PR(s)/evidence, or open a pmm-qa issue summarizing the reproduction — ask which channel is preferred if unsure) and stop.
     - **Test bug** (pmm-qa's own code is wrong — a selector, a fixture, timing, setup, an out-of-scope or stale assertion) → if this came from the **FB source**, also check whether the test's new expectation matches an **open, not-yet-merged** PR in `percona/pmm` or `percona/grafana` (search open PRs there for one touching the same area) — a submodules test occasionally gets updated ahead of the upstream change it's testing for. If such a PR exists → it's a **blocked fix**; continue to step 4, but see the draft-PR handling in step 5. Otherwise → an ordinary test bug; continue to step 4.
