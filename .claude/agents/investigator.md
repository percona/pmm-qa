...
2. **Reproduce** — Follow `linode-provisioning` to bring up a throwaway Linode VM at the given ref, and run the exact command(s) or tests that failed, or walk through the described scenario step by step if this is a question from someone. Watch what actually happens — a question only skips reproduction when reading the code makes the answer completely unambiguous and needs zero investigation; memory alone never counts.
