...
| Skill | Path |
| ------- | ------ |
| Jira read/write, visibility | `.claude/skills/relay/SKILL.md` + `references/jira.md` |
| FB checks, JNKPercona | `.claude/skills/fb-tests/SKILL.md` |
| Test design and scope | `.claude/skills/test-cases/SKILL.md` (`scope.md`, `ha-scope.md`) |
...
| Deep direct verification (API/CLI/logs/metrics/state) | `.claude/skills/test-cases/references/verification.md` |
...
Read each file when its step needs it, once — a skill or sub-agent you invoke later inherits this session, so don't re-read what's already loaded. Do not guess field IDs or setup commands.

## Workflow

1. **Read ticket** — Use the relay Jira `read` action for summary, description/AC, `customfield_10083`, `customfield_10492`, components, labels, fix versions, and comments. The relay cannot read the Development panel, so take PR URLs from ticket text and use repository searches for the rest. Cross-check every implementation PR with `git-diff` (and `json-diff` for JSON dashboard changes).
2. **Plan** — Read `test-cases` and use its candidate rules, strong-case gate, coverage search, `scope.md`, and `ha-scope.md` when applicable to produce the case set and scope that drive this run — hand it step 1's ticket context and PR diffs so it re-fetches neither; it returns the draft without publishing, and Zephyr writes happen in step 8. Add `DOCKER_ENV_VARIABLE`, `CLIENTS` / DB needs, post-provision steps, and FB images from the latest JNKPercona comment on the linked pmm-submodules PR (`gh` only). Record the deployment mode, extra dimensions, and causally linked regressions with one-line reasons. For direct API/CLI/log/metric/state requirements, read `.claude/skills/test-cases/references/verification.md` directly and create a falsifiable checklist with the owning layer, failure condition, command, and bounded window before provisioning. Declare any single observation window over 10 minutes.
...
4. **Execute** — `run.sh` for API/CLI/DB checks, applying `.claude/skills/test-cases/references/verification.md` directly; local Playwright/Chromium for UI per `ui-evidence` (use the box's plain, non-`exec-`-prefixed hostname — see `linode-docker-provisioning`'s "Accessing the VM"). Do not call a first successful snapshot "verified" when the claim is asynchronous, persistent, or spans another layer.
...
- Open PRs to `percona/pmm`, `percona/grafana`, `percona/percona-helm-charts`
