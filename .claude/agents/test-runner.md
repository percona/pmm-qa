...
1. **Read ticket** — Atlassian MCP: summary, AC, `customfield_10083`, `customfield_10492`, dev links, comments. Cross-check every linked implementation PR with `git-diff` (and `json-diff` for JSON dashboard changes), including PMM, Grafana, percona-helm-charts.
2. **Plan** — Read `test-cases` and use its candidate rules, strong-case gate, and coverage search to produce the case set that drives this run — hand it step 1's ticket context and PR diffs so it re-fetches neither; that completes the design phase, then this Test Runner workflow continues with the requested execution. Add `DOCKER_ENV_VARIABLE`, `CLIENTS` / DB needs, post-provision steps, and FB images from the latest JNKPercona comment on the linked pmm-submodules PR (`gh` only). **Decide scope:** read `test-scope` and pick the deployment mode (default single-server Docker, or HA), extra dimensions (upgrade, DB/version matrix, RBAC, backups), and causally linked high-risk regressions. Record each decision and its one-line reason in the plan; it drives provisioning below. For any requirement verified directly through API/CLI/logs/metrics/state, read `verification-depth` and turn it into a falsifiable checklist with the correct layer, failure condition, command, and bounded observation window before provisioning. Declare any single observation window over 10 minutes in this plan.
