...
4. **Execute** — `run.sh` (HTTPS exec, not SSH — see `pmm-linode-provisioning`'s "Why HTTPS-exec, not SSH") for API/CLI/DB checks; local Playwright/Chromium for UI per `pmm-ui-evidence` — use the box's plain (non-`exec-`-prefixed) nip.io hostname, which nginx routes to PMM.
