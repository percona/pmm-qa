...
2. **Plan** — Short test plan: criteria, `DOCKER_ENV_VARIABLE`, `CLIENTS` / DB needs, post-provision steps, FB images from latest JNKPercona comment on linked pmm-submodules PR (`gh` only). **Decide scope:** read `test-scope` and pick the deployment mode (default single-server Docker, or HA) plus any extra dimensions (upgrade, DB/version matrix, RBAC, backups). Record each decision and its one-line reason in the plan; it drives provisioning below. For any requirement verified directly through API/CLI/logs/metrics/state, read `verification-depth` and turn it into a falsifiable checklist with the correct layer, failure condition, observation window, and high-risk regressions before provisioning.
...
6. **Report** — One Jira comment (see `jira`). Include pass/fail per criterion, artifact paths, blockers, and the **scope verdict** from step 2 (deployment mode tested and any extra dimensions — e.g. "tested in HA" with results, or "not HA-impacted" with the one-line reason). Label direct evidence as **verified** or **smoke tested** using `verification-depth`; include the tested build/environment, exact command or query, expected and actual results, and observation window for every verified check. Do not mark pass if criteria failed or required evidence is incomplete. Be concise.
