...
1. **Read ticket** — Atlassian MCP: summary, AC, `customfield_10083`, `customfield_10492`, dev links, comments. Cross-check with `git-diff` (and `json-diff` for JSON dashboard changes) on linked `percona/pmm` / `percona/grafana` PRs.
   ...
   **A claim the change's own documentation makes about a third-party component — VictoriaMetrics, ClickHouse, Grafana, an operator — is a candidate to check, not a contract.** Confirm it against that component's published docs before using it as an expected result or as grounds to drop a case. One PR stated that VictoriaMetrics removes metrics "one whole month partition at a time", so a sub-month retention "does not remove anything from the current month"; that was used as the expected result and to tell the user the ticket's 2-day-retention scenario could not pass on any build. VictoriaMetrics' own sizing guide says the cycle "is one day or one month. If the retention period is higher than 30 days cycle is a month; otherwise day" — every period the feature uses purges daily. The user pushed back and the acceptance case had to be added afterwards.
