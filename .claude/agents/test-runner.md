...
| Is HA impacted? + PMM HA on Linode LKE | `.claude/skills/linode-ha-provisioning/SKILL.md` |
...
2. **Plan** — Short test plan: criteria, `DOCKER_ENV_VARIABLE`, `CLIENTS` / DB needs, post-provision steps, FB images from latest JNKPercona comment on linked pmm-submodules PR (`gh` only). **Decide HA:** read `linode-ha-provisioning`'s [ha-impact.md](../skills/linode-ha-provisioning/references/ha-impact.md) — does the ticket flag HA, or does the diff touch the HA blast radius (leader-elected services, shared/externalised state, VMAgent scraping, Grafana clustering, `PMM_HA_*`, the `pmm-ha` charts/operators)? Record the decision (and why) in the plan; it affects provisioning below.
3. **Provision** — Follow `linode-provisioning` for the normal single-server run. If step 2 found HA is impacted, **also** provision an HA cluster per `linode-ha-provisioning` and exercise the change there (leader failover for leader-only work, shared-state checks). If a fix under test isn't on `main` yet, push it to a branch first and pass it as `PMM_QA_REF` — never edit files directly on the box.
...
6. **Report** — One Jira comment (see `jira`). Include pass/fail per criterion, artifact paths, blockers, and the **HA verdict** (tested in HA — with results — or "not HA-impacted" with the one-line reason). Do not mark pass if criteria failed. Be concise.
