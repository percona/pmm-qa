...
4. **Execute** — `run.sh` (HTTPS exec, not SSH — see `pmm-linode-provisioning`'s "Why HTTPS-exec, not SSH") for API/CLI/DB checks. UI verification via local Playwright/Chromium (`pmm-ui-evidence`) is currently **blocked** — PMM's UI has no controller-reachable port in this setup (see the skill's step 4 UI caveat) — so if AC genuinely needs a browser check, say so explicitly in the Jira report rather than skipping it silently; don't fake a screenshot-based pass.
