...
1. **Read ticket** — Atlassian MCP: summary, AC, `customfield_10083`, `customfield_10492`, dev links, comments. Cross-check with `git-diff` (and `json-diff` for JSON dashboard changes) on linked `percona/pmm` / `percona/grafana` PRs.
   ...
   **A claim the change's own documentation makes about a third-party component — VictoriaMetrics, ClickHouse, Grafana, an operator — is a candidate to check, not a contract.** Confirm it against that component's published docs before using it as an expected result, and never let it be the reason a case is dropped. One PR stated that VictoriaMetrics removes metrics "one whole month partition at a time", so a sub-month retention "does not remove anything from the current month"; that was used as the expected result and to tell the user the ticket's 2-day-retention scenario could not pass on any build. VictoriaMetrics' own docs do not support that: its retention cycle is a day below a 30-day period, and the retention watcher removes parts far more often than monthly.

   The symmetric error is as easy to make: the sizing guide's day/month cycle appears in a **disk-space formula** and is not a promise that every sample disappears at a day boundary either — a part is removed only once all of it falls outside the window, and background merges can delay reclamation further. So neither reading settles it. Where a doc claim would decide whether a case is even runnable, **observe the case** on the version under test rather than arguing it from documentation; the user pushed back here and the acceptance case had to be added afterwards.
2. **Plan** — Short test plan: criteria, `DOCKER_ENV_VARIABLE`, `CLIENTS` / DB needs, post-provision steps, FB images from latest JNKPercona comment on linked pmm-submodules PR (`gh` only). **Decide scope:** read `test-scope` and pick the deployment mode (default single-server Docker, or HA), extra dimensions (upgrade, DB/version matrix, RBAC, backups), and causally linked high-risk regressions. Record each decision and its one-line reason in the plan; it drives provisioning below. For any requirement verified directly through API/CLI/logs/metrics/state, read `verification-depth` and turn it into a falsifiable checklist with the correct layer, failure condition, command, and bounded observation window before provisioning. Declare any single observation window over 10 minutes in this plan.
...
4. **Execute** — `run.sh` for API/CLI/DB checks, applying `verification-depth` to direct checks; local Playwright/Chromium for UI per `ui-evidence` (use the box's plain, non-`exec-`-prefixed hostname — see `linode-docker-provisioning`'s "Accessing the VM"). Do not call a first successful snapshot "verified" when the claim is asynchronous, persistent, or spans another layer.
...
6. **Report** — One concise Jira comment (see `jira`). Include the **scope verdict** from step 2, build/environment/deployment once, artifact paths and blockers, then reuse the `verification-depth` checklist as one compact row per check. Use exactly `VERIFIED`, `SMOKE TESTED`, `FAILED`, or `BLOCKED`; do not mark pass when evidence is incomplete.
...
8. **Automation decision** — After manual QA: if the change added functionality (or something else we don't yet have pipeline coverage for), and the setup isn't a wholly different pattern from what's already covered, and a `pmm-qa` test adds clear value, implement it as a Playwright test (CLI or UI) and open a PR to `percona/pmm-qa` only. Don't write new tests in CodeceptJS. **Every new test needs its own Zephyr test case** — use the `zephyr` skill to `create` one (status `Automated`), and put the returned `PMM-Txxxx` in the test title (`PMM-Txxxx - description @tag`); without a key the test reports to nothing. Don't search for an existing case first — a new test gets a new case. Otherwise stop after the Jira comment.

   Three things to settle before opening that PR:

   - **Does what the test asserts actually ship in the image its job runs?** QA of an unmerged product PR produced a test asserting a dashboard panel that exists only on that PR's feature build, wired to a job defaulting to `perconalab/pmm-server:3-dev-latest` — i.e. `percona/pmm` main, where the panel is still the pre-change one — so on merge it would have been red on every run. Confirm the change is merged to `percona/pmm` main (or present in the image the chosen tag's job pulls). When it is not, open the PR as a **draft** with a `> [!WARNING]` block naming the upstream PR, and gate the test through `helpers/versionGates.ts` keyed by its `PMM-Txxxx` once the shipping version is known.
   - **Grep the *stem* of a new tag across both suites before coining one.** CodeceptJS orders these DB-first (`@pdpgsql-pmm-integration`) and Playwright pmm-first (`@pmm-psmdb-integration`), so a new `@pmm-pdpgsql-integration` was confirmed absent as an exact string while `main` already carried the same database one word swap away. Reject any name differing from an existing tag only by word order or a separator: `runner-e2e-tests-playwright.yml:220` runs `--grep "$PMM_TEST_FLAG" … || true`, so the collision runs zero tests and reports green rather than failing loudly.
   - **A new tag with no existing job gets its own job**, named after what it runs, and added to `notify_investigator.needs` — `.claude/hooks/lint-changed.sh` rejects a new job without that, because its failures would never reach Investigator. Widening another job's `pmm_test_flag` is only for a tag that genuinely belongs to that job's subject; reusing a runner because it happens to provision the right database is not a reason, and both the reviewer and the maintainer objected independently that a red `Inventory|Image renderer tests` points at the wrong suite.
