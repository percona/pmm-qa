...
| Zephyr test-case creation | The [`zephyr` skill](../../skills/zephyr/SKILL.md) reaches Zephyr Scale through `POST /zephyr/<action>` (`search-testcase`, `create-testcase`, `get-testcase`, `list-folders`), so a new automated test can be born with its own `PMM-Txxxx`. Needs **`ZEPHYR_PMM_API_KEY`** in the relay `.env` — a Zephyr Scale API key for the `PMM` project, the same value CI already holds as a GitHub secret. Without it the broker answers `503 zephyr_not_configured` and nothing else is affected. Mirror it in the LastPass **PMM** `.env` note. |
