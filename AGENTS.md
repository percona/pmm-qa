...
- Don't write a command into a skill or agent on a reviewer's or vendor's say-so — run it in the environment that file targets first. Where it can't be run there, record it as unverified and say what to check on the first real run; the next session reads an unverified command as confirmed, which is worse than an acknowledged gap
