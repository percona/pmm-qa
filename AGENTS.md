...
No group covers `*.md`, so the dispatcher selects nothing for a docs-only diff and exits 0: report that as "no gate covers this change", never as "linter clean".

The linters do not model `set -u`/`set -e` **runtime** aborts: `make syntax`, `shellcheck -x -S warning` and bats all pass a trap that reads `${pids[index]}` on an unset subscript, which bash 5.2 aborts with `unbound variable` before the trap's `wait`, `rm -rf` and `exit 130`. Use the `:-` form on an indexed-array read whose subscript may be unset, and *execute* every new error or cleanup path (a throwaway probe script is enough) rather than reasoning about it.
...
- Set **both** git identity fields on a fresh worktree before committing — `git -C "$WT" config user.name` and `git -C "$WT" config user.email`, copied from the author's previous commit; a lone `-c user.name=…` leaves the container's default email on the commit, and the amend that would fix it is denied as destructive
