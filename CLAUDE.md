...
- **A local tooling workaround never mutates a tracked file.** Copy it to the scratch directory, patch the copy, and point the tool at it (`--config`, `--project`, …) — editing a tracked lint or build config in place changes what everyone else runs.
