    ...
    if  [[ "${CLIENT_NAME}" == "mo" && -z $MONGOMAGIC ]]; then
      ...
      for j in `seq 1  ${ADDCLIENTS_COUNT}`;do
        ...
        if [[ -z $use_socket ]]; then
          if [ $(( ${j} % 2 )) -eq 0 ]; then
            if [[ ! -z $metrics_mode ]]; then
              pmm-admin add mongodb --cluster $modb_service_name --username=mongoadmin --password=secret --metrics-mode=$metrics_mode --environment=modb-prod $modb_service_name --debug localhost:$MODB_PORT
            else
              pmm-admin add mongodb --cluster $modb_service_name --username=mongoadmin --password=secret --environment=modb-prod $modb_service_name --debug localhost:$MODB_PORT
            fi
          else
            if [[ ! -z $metrics_mode ]]; then
              pmm-admin add mongodb --cluster $modb_service_name --username=mongoadmin --password=secret --metrics-mode=$metrics_mode --environment=modb-dev $modb_service_name --debug localhost:$MODB_PORT
            else
              pmm-admin add mongodb --cluster $modb_service_name --username=mongoadmin --password=secret --environment=modb-dev $modb_service_name --debug localhost:$MODB_PORT
            fi
          fi
        else
          if [ $(( ${j} % 2 )) -eq 0 ]; then
            if [[ ! -z $metrics_mode ]]; then
              pmm-admin add mongodb --cluster $modb_service_name --username=mongoadmin --password=secret --metrics-mode=$metrics_mode --environment=modb-prod $modb_service_name --socket=/tmp/modb_${MODB_PORT}/mongodb-27017.sock --debug
            else
              pmm-admin add mongodb --cluster $modb_service_name --username=mongoadmin --password=secret --environment=modb-prod $modb_service_name --socket=/tmp/modb_${MODB_PORT}/mongodb-27017.sock --debug
            fi
          else
            if [[ ! -z $metrics_mode ]]; then
              pmm-admin add mongodb --cluster $modb_service_name --username=mongoadmin --password=secret --environment=modb-dev $modb_service_name --metrics-mode=$metrics_mode --socket=/tmp/modb_${MODB_PORT}/mongodb-27017.sock --debug
            else
              pmm-admin add mongodb --cluster $modb_service_name --username=mongoadmin --password=secret --environment=modb-dev $modb_service_name --socket=/tmp/modb_${MODB_PORT}/mongodb-27017.sock --debug
            fi
          fi
        fi
