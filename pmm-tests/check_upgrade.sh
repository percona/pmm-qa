...
if [ "$DISTRIBUTION" == "ami" ]; then
	...
	if [ "$PREPOST_UPGRADE" == "afterUpgrade" ]; then
		...
		sudo victoriametrics --version | grep victoria-metrics-20220721-122019-pmm-6401-v1.77.1
	fi
else
	...
	if [ "$PREPOST_UPGRADE" == "afterUpgrade" ]; then
		...
		docker exec $PMM_SERVER_DOCKER_CONTAINER victoriametrics --version | grep victoria-metrics-20220721-122019-pmm-6401-v1.77.1
