...
Two secrets are needed, in **each** repo that fires this Routine (`percona/pmm-qa` and, once its mirror workflow merges, `Percona-Lab/pmm-submodules`):

- `INVESTIGATOR_ROUTINE_ID` — the trigger ID from the routine's "Add an API trigger" screen (`trig_01FhHBdz2yBibyVEfnG5gbQz`).
- `INVESTIGATOR_ROUTINE_TOKEN` — the bearer token from that same screen.

Both repos here are public, so nothing about the Routine's trigger endpoint (ID or URL shape) is committed in plaintext — the ID alone can't fire anything without the token, but keeping it out of the diff too means a `git blame`/search of either repo's history turns up neither half. This doesn't change the actual injection-defense story: what protects Investigator from a malicious external PR isn't secrecy of the ID, it's (a) `notify-investigator.yml`'s `head_branch == default_branch && event == 'schedule'` guard, which already keeps a fork-PR-triggered run from ever reaching the `curl` call, and (b) the fired `text` payload only ever containing the trusted `workflow_run.name`/`workflow_run.html_url` fields, never raw PR titles/branch names/commit messages. **The pmm-submodules mirror workflow (PR #4511) needs the same two guarantees before it merges** — confirm it checks an equivalent trusted-source condition and only forwards the PR number + run URL, not any attacker-editable string, into the fired text.

**Wiring status**: `percona/pmm-qa` has both secrets set. `Percona-Lab/pmm-submodules` — confirm PR #4511 is merged and both secrets are set there too before checking this off below.
...
- [x] `INVESTIGATOR_ROUTINE_ID` and `INVESTIGATOR_ROUTINE_TOKEN` added as repo secrets in `percona/pmm-qa` so `notify-investigator.yml` can actually fire (both are secrets, not just the token — see "Investigator — event-triggered from both sources" above for why)
...
- [ ] Notify workflow added in `Percona-Lab/pmm-submodules` firing the same Investigator Routine on FB Tests red — **PR open**: [Percona-Lab/pmm-submodules#4511](https://github.com/Percona-Lab/pmm-submodules/pull/4511), watches the real "FB Tests" workflow's `workflow_run` conclusion (confirmed against that repo's actual CI, not assumed), resolves the PR number with a commit-SHA fallback for when `workflow_run.pull_requests` is empty. Before merging, confirm it reads the routine ID from a secret (not hardcoded) and only forwards the PR number + run URL — never a raw PR title/branch/commit message — into the fired text, matching pmm-qa's own hardening (see above). Needs merging, then its own `INVESTIGATOR_ROUTINE_ID` + `INVESTIGATOR_ROUTINE_TOKEN` secrets added in that repo (secrets aren't shared across repos/orgs, even with the same values).
