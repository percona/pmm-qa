...
- [x] Connector MCP tools (Atlassian Rovo, Slack, Claude Code Remote) allowlisted so headless sessions don't stall on permission prompts — **the rule name matters**: claude.ai connector tools match permission rules as `mcp__claude_ai_<server>` ([docs](https://code.claude.com/docs/en/permissions#mcp-permission-rules): "Connector tools appear as `mcp__claude_ai_<server>__<tool>`"), *not* the `mcp__<server>` display name a session shows. The original `mcp__Atlassian_Rovo`/`mcp__Slack`/`mcp__Claude_Code_Remote` entries were silent no-ops (every call still prompted → auto-deny in a headless Routine); only `mcp__github` worked because that server is project-configured, not a connector. Verified live in a web session: with `mcp__claude_ai_Atlassian_Rovo` / `mcp__claude_ai_Claude_Code_Remote` in `permissions.allow`, `atlassianUserInfo` and `list_repos` run prompt-free (exact-case server segment; settings.json edits hot-reload mid-session). Caveat: if the org later sets a connector tool to `ask` in admin settings, allow rules stop working for it by design — that would be an admin-side change, not a repo fix.
