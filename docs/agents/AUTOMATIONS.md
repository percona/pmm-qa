...
- **`/zephyr/<action>`** — Zephyr Scale test cases for the `PMM` project (project forced): `search-testcase` (relay-side ranked name scan — the v2 API has no text search), `create-testcase` (returns the new `PMM-Txxxx`), `get-testcase`, `list-folders`. Read + create only: no update, no delete, and no execution reporting (CI posts its own executions). Needs `ZEPHYR_PMM_API_KEY` in the relay `.env`; answers `503 zephyr_not_configured` without it. Used by the `zephyr` skill so a new automated test is born with a real test case key.
