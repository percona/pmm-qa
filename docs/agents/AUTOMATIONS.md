...
- [ ] Onboard each teammate: they create their own Routine(s) + API token in their claude.ai and send slack ID, jira accountId, routine id+token — becomes one small `people/<name>.json` on the relay (hot-reloaded, no restart; template in `.claude/integrations/slack/relay/person.example.json`), mirrored as a Secure Note in the LastPass PMM folder. They do NOT connect personal Slack/Jira MCP connectors — Slack replies go through the relay bot, Jira posts go through the shared REST token in the environment. Their routine runs in **their own personal Custom environment** (next item).
- [ ] **Onboard everyone on a personal Custom environment** — one org-shared environment for the whole team is **not possible right now**: shared environments only expose Trusted/Full, no Custom/Allowed-domains ([claude-code#82284](https://github.com/anthropics/claude-code/issues/82284), tracked in the findings log). So until that's fixed **or we decide Full is acceptable**, each teammate creates a **personal Custom environment** with the identical config below and points their Routine at it. Downside: N copies to keep in sync by hand; that's the cost of the bug.
  - **Network**: **Custom**, "Also include default list" checked, plus our non-Trusted hosts: `perconadev.atlassian.net`, `api.linode.com`, `*.nip.io`, `registry.terraform.io`, and the relay host `139-162-176-43.ip.linodeusercontent.com` (Slack path). Slack itself is **not** needed. (Full would also work but widens egress for everyone — that's the open decision.)
  - **Env vars (all plaintext-visible to env users → use least-privilege service credentials)**: `LINODE_TOKEN` (VM provisioning), `JIRA_EMAIL` + `JIRA_API_TOKEN` (curl-first Jira posting). Do **not** set `PMM_AI_SLACK_BOT_TOKEN` — stale: the Slack bot token lives ONLY on the relay server, fired sessions never use it.
...
- [ ] **Decide: is `Full` network acceptable for a shared env?** If yes, one org-shared Full environment removes the N-copies burden today (at the cost of unrestricted egress for every session in it). If no, stay on per-person Custom until #82284 lands. Record the decision here.
...
- [ ] **Org-shared environments can't be Custom — tracked upstream** ([claude-code#82284](https://github.com/anthropics/claude-code/issues/82284), **open**: shared/org cloud environments expose only Trusted/Full, no Custom/Allowed-domains, contrary to the documented field parity). Consequence: we can't ship **one** org-shared Custom environment carrying our allowlist (`perconadev.atlassian.net`, `api.linode.com`, `*.nip.io`, `registry.terraform.io`, relay host), so the whole team can't run identically from a single admin-managed env. **Workaround in place until fixed or we accept Full**: each teammate onboards on their **own personal Custom environment** with the same setup script + env vars + allowlist (see the onboarding checklist above). **Recheck periodically** — when a shared environment can be set to Custom, consolidate everyone onto one org-shared Custom env (admin at claude.ai/admin-settings) and retire the per-person copies; then a human flips this box and updates the onboarding item. Do **not** switch the team to a shared `Full` env just to avoid the N copies without an explicit decision — Full widens egress for every session in it.
