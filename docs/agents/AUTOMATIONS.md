...
- [ ] Atlassian Rovo / Slack connectors attached to each Routine — **previously marked done, but unverified and now contradicted by evidence**: a Test Runner run on PMM-15188 stalled on "This connector call requires your approval to proceed" for `getAccessibleAtlassianResources`. Per the [Routines docs](https://code.claude.com/docs/en/routines), a Routine run has *no approval prompts* for connectors **included in the Routine's connector list** ("Claude can use every tool from an included connector, including writes, without asking for permission during a run") — so a prompt during a run means the connector is missing from that Routine's grant. **Fix (owner-only, web UI)**: open [claude.ai/code/routines](https://claude.ai/code/routines) → each Routine (Test Runner, Investigator, FB Reporter) → edit → **Connectors** section → confirm Atlassian Rovo and Slack are listed, add them if not, save, then **Run now** to verify a run completes with zero prompts. Connector lists are only visible/editable in the web UI (the `list_triggers` MCP tool doesn't return them). Note: this only governs Routine-fired runs — a session started manually and asked to "act as Test Runner" is interactive and will always prompt (see the connector-permissions item below).
