...
- [ ] **Jira connector broken in Routine runs** ([claude-code#61015](https://github.com/anthropics/claude-code/issues/61015): approval demanded despite the connector being attached; closed upstream but still reproduced 2026-08-06 on PMM-15188). Workaround shipped: the `jira` skill is **curl-first** (REST, no approval gate); `JIRA_EMAIL` + `JIRA_API_TOKEN` added to the qa-linode environment 2026-08-06. Both connector rule spellings are kept duplicated in settings.json (`mcp__Atlassian_Rovo` and `mcp__claude_ai_Atlassian_Rovo` etc.) so whichever the platform honors post-fix is already covered. **To do**: comment on #61015 with the repro and ask for a reopen.
