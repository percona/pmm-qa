...
- [x] `gh --version`, `terraform version`, `ffmpeg -version`, and `json-diff` succeed after a fresh SessionStart hook run (`json-diff` has no `--version` flag — verify functionally with `json-diff a.json b.json`, which exits non-zero when the files differ)
- [ ] **Routine-fired sessions must be single-repo (root = `percona/pmm-qa`) for `.claude/settings.json` to load at all.** Claude Code loads project settings/hooks from the workspace **root**. In a **multi-repo** session (e.g. `pmm-qa` + `pmm` + `grafana` + `pmm-submodules` + `jenkins-pipelines` all checked out under a common `/home/user`), the root is that shared parent — there is no `.claude/settings.json` there, so `pmm-qa/.claude/settings.json` (a subdirectory) is **not loaded**: the PreToolUse `git clone pmm-submodules` guard does not fire, the SessionStart installer does not run, and the permissions allowlist does not apply. Verified: a live Bash `git clone https://github.com/Percona-Lab/pmm-submodules` was **not** blocked in a multi-repo session, even though the hook script and its self-locating wrapper both work when run by hand. The self-locate change (dropping the `$CLAUDE_PROJECT_DIR` dependency) only fixes the case where the hook *command is registered but can't find its script* — it cannot help when `settings.json` itself is never loaded. Action: ensure the Test Runner / Investigator / FB Reporter Routines launch with `percona/pmm-qa` as the **sole/root** source (they operate on `pmm-qa` and reach the other repos via `gh`/MCP anyway — see the `repos` skill), or, if a multi-repo layout is required, have the environment place the settings/hooks at the workspace root.
...
- [x] Confirm cross-org reads of `Percona-Lab/pmm-submodules` and `Percona-Lab/jenkins-pipelines` (a separate org from `percona/*`) work. **Reads confirmed** from a session that has those repos in scope: `gh api repos/Percona-Lab/pmm-submodules` and `.../jenkins-pipelines` both return, an open PR (`.../pulls`) reads, and the GitHub **MCP** tools (`pull_request_read`, `get_file_contents`) read both too. The earlier 403 ("access to this repository is not enabled for this session") came from a session where those repos were **not in scope** — the fix is to get them in scope, not a permissions grant (both repos are public and the agent proxy serves public reads). Two caveats to know: (1) `gh auth status` still reports the `GH_TOKEN` invalid even while `gh api` reads succeed — the reads go through the agent proxy, not `gh`'s own token — so do **not** treat `gh auth status` as the access oracle (see the `repos` skill's Auth note). (2) Because `gh`'s token is invalid, `gh` **write** ops (`gh pr create`, `gh pr comment`) will fail; agents that open PRs / post comments must go through the GitHub **MCP** tools (authenticated separately) or `git push` (proxy-authenticated). Still worth one confirmation from a real Routine-fired Investigator/FB Reporter run, since a Routine session's scope may differ from an interactive chat's.
