...
- **pmm-qa's own scheduled CI**: a final `notify_investigator` job in each watched suite — `e2e-tests-matrix.yml`, `gssapi-psmdb-tests-matrix.yml`, `helm-tests.yml`, `integration-cli-tests.yml` (native GitHub Actions cron), plus `nightly-e2e-tests-matrix.yml` (dispatched daily by the Jenkins pipeline in `jenkins-pipelines`). Each job `needs` every other job in its workflow and calls the shared [`.github/actions/notify-investigator`](../../.github/actions/notify-investigator/action.yml) composite action, which holds the routine ID and the fire call. This replaced a standalone `workflow_run` watcher: that watcher started a run of its own after *every* completed run, so a green night left a skipped "Notify Investigator" row in the Actions list — pure noise. `if: !cancelled() && contains(needs.*.result, 'failure')` is the job-graph equivalent of the watcher's run-level `conclusion`: it still catches a pipeline that passes its e2e-test step but fails overall once a later Launchable step errors collecting results, because that job's own result is still `failure`. The cost is that `needs` is hand-maintained — **a job added to one of those workflows must be added to its `notify_investigator` needs list**, or its failures go unreported.
- **Guards**: the four cron suites fire only on `github.event_name == 'schedule'`, so a PR run, a human's manual dispatch, and a call from `rc-testing-suite.yml` / `fb-integration-suite.yml` / pmm-submodules all stay silent. `nightly-e2e-tests-matrix.yml` has no cron of its own, so it fires on any failed run of the default branch, gated on `github.workflow` matching its own name to exclude nested reusable-workflow calls.
...
- `INVESTIGATOR_ROUTINE_TOKEN` — the bearer token from the routine's "Add an API trigger" screen, a repo secret in both repos. The routine ID itself (`trig_01FhHBdz2yBibyVEfnG5gbQz`) is hardcoded in the composite action and in the submodules job — it isn't sensitive, only the token is.

**Both sides are wired up** — the notify jobs are in this repo's suites with `INVESTIGATOR_ROUTINE_TOKEN` set as a repo secret, and the pmm-submodules side is in place too.
...
```mermaid
flowchart LR
    A1["pmm-qa's own scheduled\nCI fails on main"] -.->|"notify_investigator job\nfires the Routine"| C
...
- [x] `INVESTIGATOR_ROUTINE_TOKEN` added as a repo secret so the `notify_investigator` jobs can actually fire
