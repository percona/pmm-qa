...
- [ ] **Jira connector broken in Routine runs** ([claude-code#61015](https://github.com/anthropics/claude-code/issues/61015): approval demanded despite the connector being attached; closed upstream but still reproduced 2026-08-06 on PMM-15188). Workaround shipped: the `jira` skill is **curl-first** (REST, no approval gate). **To do**: add `JIRA_EMAIL` + `JIRA_API_TOKEN` to the qa-linode environment (plaintext-visible, same caveat as `LINODE_TOKEN`); comment on #61015 with the repro and ask for a reopen.
...
- [ ] Add `Percona-Lab/pmm-submodules` to the Investigator and FB Reporter Routines' repository lists at [claude.ai/code/routines](https://claude.ai/code/routines) — this is what enables FB Tests CI re-runs in their runs (see multi-repo item above). Archive the throwaway `qa-settings-test` environment while there.
