...
One branch, one PR, one run: today's lesson branch is the only input. The publisher reads its entries, applies the worthwhile ones **on that same branch**, validates each changed target, deletes every entry it acted on in the same commits, and opens the branch's single PR against `main`. The branch is the PR head, so it is never deleted by the run — it goes away when the PR merges.
