...
| [skill-gardener-publisher](../../.claude/agents/skill-gardener-publisher.md) | Today's `skill-gardener/<YYYY-MM-DD>` lesson branch | Scheduled — a daily Routine | Reviews today's entries, applies the worthwhile ones to skills, agents, hooks, or instruction docs **on that same branch** and deletes the entries it acted on, then opens the branch's single PR against `main` | Open a PR on a day with no lesson branch, read a branch dated other than today, delete the branch (it's the PR head), merge or approve anything, change product or test code |
| [test-health-maintainer](../../.claude/agents/test-health-maintainer.md) | The CloudBees Smart Tests workspace (every suite's recorded history) | Scheduled — a weekly Routine, Monday mornings | Folds the last 4 weeks of recorded sessions into quarantined / noisy / dead-weight / slow, ranks by how much worse each got, opens **at most one** PR of evidence-backed stabilisations, and posts a **Test Health Digest** to Slack `#qa-automation` via the relay bot | Change quarantine state in CloudBees, merge or approve anything, reproduce more than once a week |
...
A scheduled daily Routine reads [`.claude/agents/skill-gardener-publisher.md`](../../.claude/agents/skill-gardener-publisher.md), which in turn follows the **Publish** section of [`skill-gardener/SKILL.md`](../../.claude/skills/skill-gardener/SKILL.md). It is the only pass that ever edits a skill, agent, hook, or instruction doc from a captured lesson — user sessions only *capture* entries onto that day's `skill-gardener/<YYYY-MM-DD>` branch and stop, opening no PR.

One branch, one PR: today's lesson branch is the input, together with any older one a blocked or failed run stranded without a PR. The publisher reads its entries, applies the worthwhile ones **on that same branch**, validates each changed target, deletes every entry it acted on in the same commits, and opens the branch's single PR against `main`. The branch is the PR head, so it is never deleted by the run — it goes away when the PR merges.
...
**Deleting is the mechanism, not cleanup.** The gardener refuses to capture a lesson it can already see in the queue, so an applied-but-undeleted entry permanently blocks that lesson from being captured again. The deletions ride in the same commits as the change, which is also what keeps `main` free of entry files. An entry left genuinely open rides to `main` and tomorrow's branch inherits it.

**Silence is the normal output.** Most days there is no lesson branch; then the run opens no PR and posts nothing. It has no Slack delivery at all — its output *is* the PR, and `pr-maintainer`'s digest reports that PR once it exists. A run that failed shows up as a lesson branch still sitting there with no PR, which the digest cannot see — the next run picks that branch up, since a branch that never got a PR is by definition one no run finished.

**Routine**: daily, in the shared qa-linode environment, fresh session per fire, prompt pointing at `.claude/agents/skill-gardener-publisher.md`. The prompt must match the one-branch-one-PR model — one carrying the old Promote step would open a competing lessons-only PR on the very branch the publisher needs for its own, and one still requiring an entry to be merged into `main` first would deadlock, since entries never reach `main` any more. Fire at **23:00 UTC — 16:00 MST / 17:00 MDT**: the latest slot that still finishes before the UTC rollover, and after the Mountain working day ends. `<YYYY-MM-DD>` is resolved in UTC by the run itself, so if the scheduler only accepts local time, 17:00 is correct while MDT is in effect and must move to 16:00 when the clocks fall back — otherwise it lands on **00:00 UTC**, the one time that must never be used: the run resolves the brand-new UTC day, finds its branch empty or absent, no-ops silently, and leaves the finished day's full branch to the stranded-branch recovery — every day, permanently a day behind. Only today's branch is read for the day's lessons, so a lesson captured after the fire time is not picked up that day; a capture in the last hour before rollover lands on the just-published branch, rides into its open PR, and is inherited by tomorrow's branch — a day late, not lost. Daily rather than weekdays-only, so a weekend lesson is not held until Monday.

## Test Health Maintainer — weekly test-health digest

A scheduled weekly Routine reads [`.claude/agents/test-health-maintainer.md`](../../.claude/agents/test-health-maintainer.md), which follows the [`test-health`](../../.claude/skills/test-health/SKILL.md) skill. Every pmm-qa CI test session is already recorded to the CloudBees Smart Tests (formerly Launchable) workspace by all four suite families, so two years of flakiness, failure-rate and duration history exists — and until this agent, **nothing read it back**: "flaky" was decided per-failure off a single FB run, with no history behind the call.

The run takes the last 4 weeks of sessions from `launchable stats test-sessions`, folds each session's `launchable inspect tests` output per test path, and sorts the result into four buckets: 🔴 **quarantined** (a `launchable gate` verdict that passes while the raw session failed), 🟠 **noisy** (both failed and passed inside the window), 🟡 **dead weight** (ran often, never failed), 🔵 **slow**. The session count travels with every number, since a bad score over three sessions is not the same finding as the same score over three hundred. "Noisy" is our word: CloudBees exposes flakiness score, failure rate, never-failing and longest, and the skill maps ours onto theirs. Cutoffs are read off the week's own distribution rather than hardcoded.

**Why folding rather than four calls.** The CloudBees docs describe a `smart-tests view flaky-tests | never-failing-tests | longest-tests | test-results` family that would do this server-side. That is a separate package and **`smart-tests` is not on public PyPI** — `pip3 install smart-tests` 404s, so it is not a version-pin problem — while the `launchable~=1.0` CLI the workflows already install (1.124.4) has no `view` command at all. Its real surface is `stats`, `inspect`, `gate`, `detect-flakes`, `subset`, `record` and `compare`. The curated trend views remain available in the web app, and the skill requires saying so when a finding came from there rather than from a command. If the `smart-tests` package becomes reachable, the folding collapses to four calls.

**Evidence before fixes.** Existing failure logs are the primary evidence and need no environment, which covers the common case — the CLI returns no log text, so they come from the session's own GitHub Actions job log (via the GitHub MCP `get_job_logs`, since Routine sessions have no `gh`) and the `logs.zip` each runner uploads with `record attachment`. Only where those are genuinely inconclusive *and* the candidate is the week's top pick does it hand that one candidate to **Investigator** — read-and-follow in the same session, the same pattern Test Runner uses for FB Reporter — reusing the existing throwaway-Linode reproduction path. **At most one reproduction per week**; that cap is what keeps a weekly Routine cheap.

Delivery goes through the relay for the same reason PR Maintainer's does: a Routine-fired session stores no MCP connectors, so there is no Slack tool in it — `RELAY_KEY` plus the caller's `X-Actor` is the whole path. The same caveat applies to reading a job log, so the skill names the GitHub MCP, `gh` and the REST API as interchangeable and treats "none of them reachable" as a finding rather than something to route around.

Output is **at most one PR** (branch `claude/test-health-<YYYY-MM-DD>`, marker `## Test health (weekly)` for dedup) carrying only changes the evidence supports, plus a **Test Health Digest** to Slack `#qa-automation` through the same relay `POST /slack/announce` path PR Maintainer uses. A week with nothing provable opens no PR and still posts a one-line all-clear, so a quiet week is distinguishable from a broken Routine. It never merges or approves, and **never changes quarantine state** — quarantining is a human decision, manual in the web UI or automatic from the failure-rate threshold config.

**Prerequisite: `LAUNCHABLE_TOKEN` in the Routine's environment.** Same CLI and same variable name CI already uses, so nothing new is pinned and nothing new is issued — but that secret is scoped to GitHub Actions, and a Routine session's environment does not have it (only `RELAY_KEY` is there today). Adding it as a Custom environment variable on the environment the Routine runs in is the one human step. It is a read-only analytics credential, so unlike the write paths it needs no relay brokering. Until it exists the Routine fires, reports the missing token, and posts nothing else — it never reports a false all-clear. (`LAUNCHABLE_ORGANIZATION` + `LAUNCHABLE_WORKSPACE` is the OIDC alternative.) If adding it is refused, the fallback is a scheduled workflow doing the reads where the secret already lives and uploading JSON the Routine consumes — strictly more machinery.

**One finding this surfaced:** quarantine currently has **no effect in pmm-qa CI**. Enforcement is a `launchable gate --session` step — already present in the CLI CI installs, so wiring it is a workflow change, not a CLI migration — and no workflow runs one; the test steps end in `|| true` ([`runner-e2e-tests-playwright.yml:220`](../../.github/workflows/runner-e2e-tests-playwright.yml)), so a test step cannot fail its job either way — and the nightly runner swallows the results upload too (`runner-e2e-tests-playwright-remote-nightly-tests.yml:297`), so a nightly week can be silently missing sessions. [`investigator.md`](../../.claude/agents/investigator.md) attributes result-muting to quarantine; the likelier cause is the `|| true`. Wiring the gate and dropping those `|| true`s is a follow-up, not part of this agent.
