...
- [ ] **Multi-repo sessions across orgs — the intended cross-org path; needs one live verification.** Findings that make this the plan of record: (1) multi-repo is official ([web quickstart](https://code.claude.com/docs/en/web-quickstart): "You can add multiple repositories to work across them in one session"; [routines docs](https://code.claude.com/docs/en/routines): "Add one or more GitHub repositories"), with **no documented same-owner restriction at creation** — the v1 cross-tier refusal is specific to the mid-session `add_repo` flow; (2) the GitHub proxy scopes API access to "repositories **attached to the session**" ([cloud-environments docs](https://code.claude.com/docs/en/cloud-environments)), so selecting both repos at creation should light up `gh api`/`gh run rerun` for both, including `Percona-Lab/pmm-submodules` CI re-runs; (3) **a personal-token workaround is impossible**: verified live that the proxy replaces whatever token `gh` sends (a deliberately bogus `GH_TOKEN` still succeeded against the in-scope repo) — session scope is decided by the proxy, never by your credential, so a `Percona-Lab` PAT in an env var does nothing for `api.github.com`. **Open question the docs are silent on**: which repo's `.claude/settings.json` (permissions + hooks) loads when the session's primary working directory is a different repo. Known from the current harness: repos added mid-session load `CLAUDE.md`/skills/plugins but *not* settings/hooks, and there is exactly one "primary working directory" — so assume hooks/permissions come **only from the primary repo**, and always select `percona/pmm-qa` FIRST when creating multi-repo sessions/Routines. The hooks themselves already self-locate with a `/home/user/pmm-qa` fallback, so they survive path differences as long as pmm-qa's settings.json is the one loaded. **Verification protocol (one multi-repo test session: `percona/pmm-qa` first + `Percona-Lab/pmm-submodules`)**: (a) `ls ~` — confirm both clones and pmm-qa's path; (b) confirm the SessionStart hook ran (`gh --version`, `terraform version` succeed); (c) confirm the PreToolUse guard blocks `git clone …pmm-submodules`; (d) `gh api repos/Percona-Lab/pmm-submodules` and `gh api repos/percona/pmm-qa` both succeed; (e) `gh run list -R Percona-Lab/pmm-submodules` then `gh run rerun <id> --failed -R Percona-Lab/pmm-submodules` on a red FB run — the actual go-live capability; (f) push access sanity: `git push` a `claude/`-prefixed test branch to pmm-qa. If (b)/(c) fail with submodules as primary but pass with pmm-qa primary, the primary-repo rule is confirmed — record it here. Once verified, add `Percona-Lab/pmm-submodules` to the Investigator and FB Reporter Routines' repository lists (edit at [claude.ai/code/routines](https://claude.ai/code/routines)) with pmm-qa kept first.
