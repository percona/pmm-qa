...
> **Status (2026-08-12):** the broker (`/<service>/<action>`) is implemented and under validation on a staging relay — endpoint consolidation, the `skip_instance_*_poll` token fix (so `/linode/destroy` works with a Linodes+Firewalls-only token), and the identity gate in **`assert` mode**. Confirmed so far: `/linode/provision` returns `{ip, exec_token, exec_cert_pem}` and the deny paths reject a missing/bad token and a wrong `RELAY_KEY`; the `assert`-mode roster checks and the `/linode/destroy` fix are in the current validation run. The unspoofable `verify` mode is coded and needs a real GitHub token to exercise (see modes below). Prod cutover (set `RELAY_KEY`/`RELAY_GH_ALLOW`/`RELAY_IDENTITY_MODE` in the qa-linode env, remove `LINODE_TOKEN`/`JIRA_*` from the shared env) is the remaining human step.
