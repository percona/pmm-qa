...
- **The Jenkins nightly orchestrator**: one `fire` at the end of `pmm3-nightly-orchestrator.groovy`'s `Report` stage, carrying every failed suite and its URL. The orchestrator is the only thing that sees the whole night — most of its ~25 lanes never touch GitHub Actions at all (`pmm3-ui-tests`, `pmm3-upgrade-test-runner`, `nightly-package-testing-*`, `pmm3-ha-tests`, `pmm3-ui-tests-nightly-gssapi`, `openshift-helm-tests`, the CLI integration jobs) and so had no path here; the two that do (`nightly-e2e-tests-matrix.yml` via `pmm3-ui-tests-nightly-gha`, and `nightly-test-suite.yml`) are waited on and fold their verdict into the same `results` map.

  This replaced a `notify_investigator` job in each watched suite. Firing per failed workflow spawned one Routine session per lane, each running its own PR-and-Jira dedup sweep and provisioning its own Linode VM — on a night whose eight red jobs shared a single upstream cause, that was eight investigations of one problem. The old design knew it: the dedup step still carries a hand-written "treat the batch as one investigation rather than one fix each" rule that existed only to paper over the fan-out. One notification per night removes the need for it, and lets the session group by cause before it reproduces anything. A failure to notify is logged and never escalated — it must not colour the report.

- **Guards**: none needed on this side any more. The suites no longer notify, so a PR run, a human's manual dispatch and a call from `fb-integration-suite.yml` or pmm-submodules are silent by construction rather than by an `if:` that had to be kept in step with how the run was triggered.

- **`Percona-Lab/pmm-submodules` FB Tests**: the same pattern mirrored in that repo — a `notify_investigator` job at the end of `.github/workflows/pmm-qa-fb-checks.yml` (the workflow named "FB Tests"), firing the same Investigator Routine with the submodules PR number + run URL. It resolves the PR from the run's head SHA, and notifies nobody when that PR is already closed or merged. (Originally a `workflow_run` watcher, [#4511](https://github.com/Percona-Lab/pmm-submodules/pull/4511); inlined for the same Actions-list-noise reason.)

Only one secret is needed:

- `INVESTIGATOR_ROUTINE_TOKEN` — the bearer token from the routine's "Add an API trigger" screen. It is a repo secret in `Percona-Lab/pmm-submodules` and a **Jenkins credential** of the same name for the orchestrator. The routine ID itself (`trig_01FhHBdz2yBibyVEfnG5gbQz`) is hardcoded in the orchestrator and in the submodules job — it isn't sensitive, only the token is.

The pmm-submodules side is wired up. The orchestrator side needs `INVESTIGATOR_ROUTINE_TOKEN` added as a Jenkins credential before it can fire.
...
```mermaid
flowchart LR
    A1["Jenkins nightly\norchestrator goes red"] -.->|"Report stage fires\nthe Routine once"| C
    A2["pmm-submodules FB Tests\ngoes red"] -.->|"notify_investigator job\nin FB Tests there"| C
...
**HA runs have a different backstop.** When a change is HA-impacted, Test Runner (or Investigator reproducing an HA failure) also stands up a Linode LKE Kubernetes cluster via [linode-ha-provisioning](../../.claude/skills/linode-ha-provisioning/SKILL.md) — a multi-node cluster plus a LoadBalancer, billed by the hour. That cluster has **no on-box self-destruct timer** (there's no box to run one), so primary cleanup is the agent calling the skill's `destroy-lke.sh` on every exit path; the relay-side TTL reaper described below deletes any expired `pmm-qa-ephemeral` cluster it misses. Whether HA is in scope is decided up front from the ticket and diff by [test-cases](../../.claude/skills/test-cases/SKILL.md) (`scope.md` and `ha-scope.md`); most changes aren't HA-impacted and never create a cluster.
...
## Skill Gardener Publisher — weekly lesson publisher

A scheduled weekly Routine reads [`.claude/agents/skill-gardener-publisher.md`](../../.claude/agents/skill-gardener-publisher.md), which in turn follows the **Publish** section of [`skill-gardener/SKILL.md`](../../.claude/skills/skill-gardener/SKILL.md). It is the only pass that ever edits a skill, agent, hook, or instruction doc from a captured lesson — user sessions only *capture* entries onto that week's `skill-gardener/<YYYY>-W<WW>` branch and stop, opening no PR.

One branch, one PR **per week**: the current ISO week's lesson branch is the input, together with any older one a blocked or failed run stranded without a PR (the same stranded-branch rule that also sweeps up the legacy day-named branches from before the weekly switch). The publisher reads its entries, applies the worthwhile ones **on that same branch**, validates each changed target, deletes every entry it acted on in the same commits, and opens the branch's single PR against `main`. The branch is the PR head, so it is never deleted by the run — it goes away when the PR merges. Weekly rather than daily because each day's lessons were only a handful of small instruction edits: batching a week into one branch and one PR cuts the review traffic without changing what gets reviewed.

**That one PR is the only gate.** There is no promotion step and no merged-entry precondition, so the publisher acts on evidence no human has read yet. Its PR review is what stands in front of an unattended instruction edit, which is why the body must carry each lesson's sanitized evidence, the change, its validation, and the reason for every lesson declined — the entry files leave with the merge.

**Deleting is the mechanism, not cleanup.** The gardener refuses to capture a lesson it can already see in the queue, so an applied-but-undeleted entry permanently blocks that lesson from being captured again. The deletions ride in the same commits as the change, which is also what keeps `main` free of entry files. An entry left genuinely open rides to `main` and next week's branch inherits it.

**Silence is the normal output.** A week with no lesson branch means the run opens no PR and posts nothing. It has no Slack delivery at all — its output *is* the PR, and `pr-maintainer`'s digest reports that PR once it exists. A run that failed shows up as a lesson branch still sitting there with no PR, which the digest cannot see — the next run picks that branch up, since a branch that never got a PR is by definition one no run finished.
...
## Review Feedback Gardener — daily human-feedback sweep (Routine live; no-ops until this agent is on `main`)
...
[`skill-gardener/SKILL.md`](../../.claude/skills/skill-gardener/SKILL.md), and applying the entries
...
so the logic stays versioned in the repo. The one constraint that matters is that Sunday's fire land
before the publisher's, so the week's last day is inside the PR the publisher opens; a comment made
after that fire rides the next run onto the following week's branch, a week late but not lost. The
prompt opens with a guard: no agent file on `main` means capture nothing and stop, so the Routine
could be created before the agent merged.

Unlike the publisher's, this Routine was created through the meta-MCP `create_trigger`, which
exposes no `sources`, `model` or `allowed_tools` — its stored `config.sources` is empty where the
publisher's names `percona/pmm-qa` explicitly. A fired session did pick up `claude-opus-5` and
`permission_mode: auto` by default, so defaults do fill in, but **whether the clone arrives the same
way is unverified**: the first fire after this merges is what proves it. If it reports no
repository, recreate the Routine from the claude.ai Routines UI, where sources are explicit.
...
- **`/jira/<action>`** — Jira ops scoped to the PMM project, forcing Developers-only comment visibility server-side: `create`, `read`, `search`, `comment`, `field` (`customfield_10492` etc.), `transitions`, `transition`, and `attach`. No delete.
...
- **`/zephyr/<action>`** — Zephyr Scale test cases for the `PMM` project (project forced): `search` (relay-side ranked name scan — the v2 API has no text search), `create` (returns the new `PMM-Txxxx`), `get`, `list`, `statuses`, `folders`, `set-status` (the Needs Automation → Automated flip), `steps`, `link-issue` (a coverage link to a PMM ticket; the relay resolves the key to the numeric Jira id Zephyr wants) and `create-folder` (a new `TEST_CASE` folder, e.g. `PMM4.x Tests` when a new major starts). **Reads resolve Zephyr's references:** the v2 API returns status, priority, folder, steps and linked issues as bare ids or URLs, so `get` adds a `resolved` block naming them (steps read from `/teststeps`, Jira `issueId`s resolved to keys through the Jira service account), `statuses` exposes the id→name tables, and `list` enumerates a folder subtree without the filler query `search` used to need. Lookup tables are cached 10 min; resolution is fail-soft, degrading to `resolved.error` rather than failing the read. No free-form edit and no delete — Zephyr has no delete at all, so retiring a case means `set-status Deprecated`; executions stay CI's job. `set-status` is a server-side read-modify-write because Zephyr's update endpoint replaces the whole test case and clears every omitted field. Needs `ZEPHYR_PMM_API_KEY` in the relay `.env`; answers `503 zephyr_not_configured` without it. Used by `test-cases` through the shared `relay` skill.
...
- **Jira: fully covered by `/jira/<action>`.** create + read + search + comment (Developers-only, forced) + field + transitions + transition + attach — the complete surface documented by the relay Jira reference. **`create`** files a new issue with the project **forced to `PMM`** (Investigator uses it to file auto-detected product bugs; the relay defaults **Found by Automation** `customfield_10059` = Yes on the Bugs it creates). Only **delete** is refused by construction.
...
- [x] Create the daily **Review Feedback Gardener** Routine — 20:00 UTC, fresh session per fire, prompt pointing at [`.claude/agents/review-feedback-gardener.md`](../../.claude/agents/review-feedback-gardener.md) (done 2026-09-09). Sunday's fire lands before the publisher's, so the week's last day makes that week's PR.
- [ ] **Confirm the fired session gets a `percona/pmm-qa` clone.** `create_trigger` stored an empty `config.sources`; the publisher's Routine names the repo explicitly. A plumbing fire on 2026-09-09 succeeded but its report was not readable from the creating session. Read the first fire after the agent lands on `main`: a run that says the agent file is missing proves the clone; a run that says there is no repository means recreate the Routine from the claude.ai Routines UI.
...
- [x] **Jira connector broken in Routine runs — workaround complete** ([claude-code#61015](https://github.com/anthropics/claude-code/issues/61015): approval demanded despite the connector being attached; closed upstream but still reproduced 2026-08-06 on PMM-15188). The relay Jira reference is **curl-first** (REST, no approval gate); `JIRA_EMAIL` + `JIRA_API_TOKEN` were added to the relay environment 2026-08-06. Both connector rule spellings remain in settings.json so whichever the platform honors post-fix is already covered.
...
- [x] **Auto-mode classifier — `autoMode` block committed to `.claude/settings.json`** (2026-08-17), so it stops blocking relay-brokered provisioning in Routine runs. Takes effect on merge to `main` + a setup-script cache bust (delivery, same as every settings change). **Decision (2026-08-17): repo→user-scope path, NOT org/managed settings.** The block goes into this repo's `.claude/settings.json`; the qa-linode setup script already copies that file to `/root/.claude/settings.json` (user scope, see line ~350), which is where the classifier reads it — so merging the block resolves this with no org-settings change. **Applied 2026-08-17:** the block is now committed to `.claude/settings.json` in this PR (with Davi's explicit authorization for this one edit; the 5 obsolete `Bash(*linode-runner/*.sh *)` allow rules were dropped at the same time). It resolves once this merges to `main` **and** the setup script is `touch`ed to bust its ~7-day cache. The block that was added is below. Hit live 2026-08-10 and again 2026-08-14 ([run 31765312223](https://github.com/percona/pmm-qa/actions/runs/31765312223)): Investigator got through dedup, then every attempt to reach the provisioning relay was denied with `Blocked by classifier`. A Routine cannot show a permission prompt, so the run just continued and closed with no verdict. Mechanics — the earlier notes here were partly wrong; **corrected against the official docs 2026-08-14** ([auto-mode-config](https://code.claude.com/docs/en/auto-mode-config), [permissions](https://code.claude.com/docs/en/permissions), [server-managed-settings](https://code.claude.com/docs/en/server-managed-settings)):
  ...
  - **An agent cannot apply the `settings.json` change itself** — editing `.claude/settings.json` is a self-escalation boundary (an agent granting itself provisioning rights). A human commits it, or an Owner pastes the `autoMode` block into managed settings. Both edits so far were made by an agent only under Davi's explicit, in-conversation authorization for that specific edit (2026-08-17 provisioning block; 2026-08-24 relay-wide entry below).
  - **Extended 2026-08-24 to cover every relay-brokered action, not just provisioning.** The first block trusted the relay's *host* and pre-approved *provisioning*, so relay **writes to other upstreams still got denied**. Hit live in a cloud session working the HA test inventory: `POST <relay>/zephyr/set-status` was denied `Blocked by classifier` twice — once for an invalid-status probe (`{"key":"PMM-T2104","status":"__probe_invalid__"}`, which the relay rejects **before** reading or writing the case, so it was read-only in effect) and once for a genuine no-op (`{"key":"PMM-T2140","status":"Automated"}`, the status that case already had, used to confirm a status id). Every read action through the same broker — `get`, `search`, `folders`, and `/jira/read`, `/jira/search` — went through untouched, and one *other* `set-status` invalid-name probe was allowed, so the classifier was judging the payload's apparent intent, not the endpoint: inconsistent per call, which is exactly what a written `allow` entry settles. Consequence while it was denied: the id→name mapping of Zephyr test-case statuses could not be confirmed by the one mechanism the broker offers (`set-status` returns `statusId` + `previousStatusId`, so applying a case's existing status names an id and changes nothing), and had to be inferred from the status list plus a proven anchor instead. That second gap — the broker returning ids nobody could resolve — is fixed in the same PR (`statuses`, `list`, and reference resolution on `get`), so a status name is now a read rather than an inference or a write.
  ...
  ```json
  {
    "autoMode": {
      "environment": [
        ...
        "Key internal services these sessions talk to: an automation relay that brokers infrastructure, ticketing and test-case actions (Linode VM and LKE provisioning/teardown, Jira, Zephyr Scale test cases, chat notifications) at a *.ip.linodeusercontent.com host whose address is not fixed; Percona Jenkins at pmm.cd.percona.com (and *.cd.percona.com); Percona Jira at perconadev.atlassian.net; Launchable at *.launchableinc.com; the Linode API at api.linode.com; the Terraform registry at registry.terraform.io; and Percona's public Docker Hub namespaces docker.io/percona and docker.io/perconalab.",
      ...
      "allow": [
        ...
        "Every action the automation relay brokers is authorized, writes as well as reads: a session POSTing to <relay-host>/<linode|jira|slack|zephyr>/<action> is doing ordinary pre-approved work, not reaching outside its environment. The relay, not the session, is the guardrail — every request carries the shared RELAY_KEY plus an X-Actor GitHub login the relay roster-checks and records, and each broker constrains its own upstream: Zephyr is forced to project PMM, assigns a created case's owner to the caller, exposes only read/create/status/steps and has no delete; Jira is forced to project PMM with Developers-only comment visibility and no delete; chat posts go only to the channels the relay is configured for; Linode provisions and destroys only the throwaway, self-expiring infrastructure described above. Test-case and ticket bookkeeping through that broker is therefore routine QA work needing no separate confirmation: flipping a Zephyr case to Automated, writing its steps, creating a case, reading a status id back by applying the status a case already has (a no-op write), creating or commenting on a Jira ticket, updating a field, transitioning it, or attaching evidence."
      ]
    }
  }
  ```
...
- [x] `INVESTIGATOR_ROUTINE_TOKEN` added as a repo secret (pmm-submodules). The orchestrator needs the same token as a Jenkins credential
...
- [x] **Multi-repo sessions load no project settings** — they root at the *parent* of the clones, so `settings.json` (hooks + permissions) loads from nowhere, though `CLAUDE.md`/agents/skills still load. **Two details here were wrong and are corrected in the 2026-08-25 entry below: the root is now `/workspace`, not `/home/user`, and `$CLAUDE_PROJECT_DIR` *is* set in the hook process (to the session root) — it is only absent from the Bash tool's environment.** **Fixed and verified end-to-end 2026-08-06**: the environment setup script below plants the committed settings at user scope (`/root/.claude/settings.json`), which cloud sessions honor — SessionStart tooling, clone guard, and allowlist all active in a fresh multi-repo session with zero prompts. **Applied to the qa-linode environment 2026-08-06.** Caveats: setup-script results are snapshot-cached ~7 days (touch the script to force a rebuild after settings changes on main); the allowlist applies to every session in that environment. Environment setup script (web-UI-only config; this snippet is the record):

  ```bash
  # SUPERSEDED 2026-08-25 -- the /home/user path no longer exists, so this always
  # fell through to the curl branch and froze a snapshot-time copy of main.
  # Replace with the version in the 2026-08-25 entry below.
  mkdir -p /root/.claude
...
- [x] Notify workflow in `Percona-Lab/pmm-submodules` firing the Investigator Routine on FB Tests red — **[#4511](https://github.com/Percona-Lab/pmm-submodules/pull/4511) merged**. Since folded into "FB Tests" itself as a `notify_investigator` job that resolves the PR number from the run's head SHA; secret `INVESTIGATOR_ROUTINE_TOKEN` in place.
...
- [ ] **Repo `.claude/settings.json` hook edits never reach cloud sessions — needs one environment-config change** (investigated 2026-08-25, in a cloud session on Claude Code **2.1.245**). Hooks work locally and are inert in the cloud for a delivery reason, not a code reason: **the hook scripts and the wrappers in `main` are both correct.** What actually happens:
  - Cloud sessions root at **`/workspace`**, with the clones at `/workspace/<repo>`. Project settings are read from **`$CLAUDE_PROJECT_DIR/.claude/settings.json` = `/workspace/.claude/settings.json`**, which does not exist — so the repo's own `/workspace/pmm-qa/.claude/settings.json` is never loaded as project settings.
  - The only hook config that does load is **`/root/.claude/settings.json`**, planted by the qa-linode setup script. Its `/home/user/pmm-qa` branch is dead (that path no longer exists), so it always takes the `curl …/main/.claude/settings.json` branch — at **snapshot-build** time. Measured: the file's mtime was `2026-08-24 11:31`, the same as `/workspace` itself, while `/proc/uptime` put the container's boot 38 s before the check — i.e. a snapshot artifact, not a per-session seed. Its content matched the repo exactly up to `6dcbae752` and diverged at [#1226](https://github.com/percona/pmm-qa/pull/1226), so every cloud session was running the **pre-#1226** two-rung wrapper (`"$CLAUDE_PROJECT_DIR"`, `/home/user/pmm-qa`) — both rungs dead — and the pre-[#1233](https://github.com/percona/pmm-qa/pull/1233) `PostToolUse`/`Skill` gardener hook. `CLAUDE_CODE_REMOTE_SKIP_SETTINGS_SYNC=1` is set, so nothing re-syncs it mid-session.
  - **`main`'s four-rung wrapper is right.** Extracted from `settings.json` programmatically and run under the conditions the hook process actually sees, every hook resolves via rung 2: `"$CLAUDE_PROJECT_DIR/pmm-qa"` → `/workspace/pmm-qa/.claude/hooks/<name>`. It only ever needed to be *loaded*.
  - **Measurement traps that made this look worse than it is** (both hit during earlier passes): `$CLAUDE_PROJECT_DIR` **is** set in the hook process (`/workspace`) even though it is absent from the Bash tool's environment — the two are different environments, so infer nothing about hooks from `env` in a Bash call. And `$CLAUDE_CODE_DIAGNOSTICS_FILE` records `hook_spawn_started` **only for `SessionStart`/`SessionEnd`** — a probe that demonstrably ran left no entry. "Zero `PreToolUse` spawns in the diag log" is therefore not evidence a hook did not fire; instrument the hook itself. Also `$CLAUDE_CODE_VERSION` was `2.1.42` while the binary at `$CLAUDE_CODE_EXECPATH` reported `2.1.245` — trust `--version`, never the variable.
  - Verified live, by instrumenting one hook and watching the guard fire: user- and project-scope hooks **merge** (both run for the same call, so a hooks block in each fires twice); settings **hot-reload** mid-session, but a project settings file created *after* startup is only picked up once a watched settings file changes; a **symlinked** `settings.json` is honored. With `/workspace/.claude/settings.json` pointing at the repo, `block-pmm-submodules-clone.sh` blocked a matching command that had gone straight through minutes earlier, and an edit to the **repo's** `settings.json` took effect in the same session — the thing that was broken.
  - **This is an environment-config change, not a repo change** — nothing in the repo can fix it, because in a cloud session no repo code runs at all: the one loaded hook config points only at dead paths, so there is no bootstrap foothold. **What must change:** the **qa-linode shared environment's setup script** (Claude Code web UI → that environment → setup script; it is not version-controlled, which is why this snippet is the record). **Who:** whoever can edit that shared environment — an admin at `claude.ai/admin-settings`; teammates in the env cannot. Replace its body with:

  ```bash
  # pmm-qa: make the repo's committed .claude/settings.json the live config for
  # cloud sessions, which root at /workspace (the parent of the clones).
  set -u
  QA=/workspace/pmm-qa

  # Hooks + permissions -- symlink, so they always match the session's own
  # checkout instead of a snapshot-time copy that can drift days behind it.
  mkdir -p /workspace/.claude
  ln -sfn "$QA/.claude/settings.json" /workspace/.claude/settings.json

  # autoMode -- the classifier reads it from user scope only. Strip hooks so
  # they do not fire a second time from here (user + project hooks both run).
  mkdir -p /root/.claude
  src="$QA/.claude/settings.json"
  if [ ! -f "$src" ]; then
    curl -fsSL --max-time 30 \
      https://raw.githubusercontent.com/percona/pmm-qa/main/.claude/settings.json \
      -o /tmp/pmm-qa-settings.json && src=/tmp/pmm-qa-settings.json
  fi
  if [ -f "$src" ]; then
    jq 'del(.hooks)' "$src" > /root/.claude/settings.json
  else
    echo "WARN: pmm-qa settings unavailable" >&2
  fi
  exit 0
  ```

  Why the symlink rather than a fresh copy: a copy is taken at snapshot time and can sit days behind the checkout the hook scripts come from — exactly this bug, where `settings.json` still said `/home/user` after the layout had moved to `/workspace`. The symlink resolves into the session's own clone, so `settings.json` and the scripts it names can never skew. `autoMode` still has to be copied to user scope because the classifier reads it from nowhere else (see the auto-mode entry above), and `del(.hooks)` keeps that copy from double-firing every hook. Editing the setup script also busts its ~7-day snapshot cache, so this lands on the next session rather than whenever the cache expires.
  **Ran and verified in-session** (2026-08-25): the script produced the symlink and a hooks-free user-scope file that kept `autoMode`/`permissions`, after which the clone guard fired from the repo's checkout. **Not verifiable from a session:** that a project settings file *present at startup* is loaded during the initial `settings_load` rather than needing a reload. The loader demonstrably reads that path, so this is expected, but it can only be confirmed by a fresh session after the setup script changes — check there that the guard fires with no manual reload.

- [x] **`ConfigChange` is a real hook event — [#1226](https://github.com/percona/pmm-qa/pull/1226) removed the gardener's `ConfigChange`/`skills` hook on a wrong premise** (corrected 2026-08-25). That PR claimed `ConfigChange` was not a valid hook event; the claim came from grepping the wrong binary — `/opt/node22/bin/claude` (2.1.239) while sessions ran `/opt/claude-code/bin/claude` (2.1.245). Grepping the binary at `$CLAUDE_CODE_EXECPATH`, confirmed `2.1.245` by `--version`, `ConfigChange` appears in the hook-event enum alongside `PreToolUse`/`SubagentStart`/`SessionStart`. **Resolve the running binary via `$CLAUDE_CODE_EXECPATH` and confirm with `--version` before trusting any string grepped out of a bundle.** Re-adding the hook was deliberately left out of the correction: [#1233](https://github.com/percona/pmm-qa/pull/1233) since moved the gardener to `UserPromptSubmit` + `SubagentStart`, so restoring a third trigger is a behaviour decision for whoever owns the gardener, not part of fixing the record.
...
| Investigator (was "Test Doctor") | `trig_01FhHBdz2yBibyVEfnG5gbQz` | Renamed, prompt updated to read `.claude/agents/investigator.md`. Same trigger ID — the notify jobs already point at it. |
