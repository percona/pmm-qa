...
**Jenkins access (trigger staging builds from agents):**

> Context: there is no need for a Jenkins API token / service account — a Jenkins **MCP gateway** already runs in production (`https://jenkins-mcp.cd.percona.com/mcp`), authenticating each person via Percona SSO + Duo so builds stay attributed to individuals (per Anderson Nogueira). The catch: it's a claude.ai **org connector**, which is the category that stalls on the host-side approval prompt in Routine runs (see the connector-prompt and classifier findings below). So the plan splits by surface.

- [ ] **Interactive sessions — add the Jenkins MCP as a claude.ai org connector** (org-owner only, in claude.ai admin settings — it's a *custom connector*, not a Jenkins plugin). Values for IT: URL `https://jenkins-mcp.cd.percona.com/mcp`, client ID `jenkins-mcp` (under Advanced settings), client secret **blank** (public OAuth client using PKCE S256; security comes from the strict redirect allowlist).
- [ ] **Unblock the connector: merge [`percona/percona-cd-platform` #403](https://github.com/percona/percona-cd-platform/pull/403)** — it adds `https://claude.ai/api/mcp/auth_callback` to the Authentik redirect allowlist. Still open, zero approvals, review requested from `@vorsel`. Critical path: the connector can't finish OAuth without it.
- [ ] **Routines — ask Anderson for a narrow per-job build token scoped to `pmm3-*`**, driven by **curl via Bash** (same curl-first pattern as the `jira` skill). Do **not** add `jenkins-mcp` to a Routine's `mcp_connections`: org connectors stall on approval in routine runs; only environment/project-provisioned MCP servers (like `mcp__github`) work there. A build token over curl sidesteps that.
- [ ] **Prerequisite for the routine path — the auto-mode classifier fix must land first** (the human-applied `autoMode.allow` edit tracked in the findings log). Kicking off a staging build (`pmm3-aws-staging-start`) is the same billable/irreversible class as `terraform apply`, so the classifier will block it in routine runs regardless of how Jenkins is reached. Routines can't drive Jenkins until that's applied.
- [ ] **Reply to Anderson** (nothing sent yet): (a) confirm whether an environment-provisioned Jenkins MCP would work inside routines, (b) ask for the per-job `pmm3-*` build token, (c) send the list of names he requested for the writers group.
- [ ] **Decide the attribution model before wiring Jenkins into unattended runs**: CI-fired routines would start builds under one human's identity (e.g. a 3am nightly under yours), and the relay's `ALLOW_FALLBACK` would collapse other people's Jenkins builds onto that same identity. Confirm with Anderson before an audit surfaces it, and keep `ALLOW_FALLBACK` **off** for anything touching Jenkins.

**Later / optional:**

- [ ] Onboard each teammate: **Slack member IDs + Jira accountIds for the whole team are already resolved** (2026-08-11, via the Slack API + Atlassian `lookupJiraAccountId` — nobody sends their own IDs; the `slack`/`jira` blocks of every `people/<name>.json` are pre-filled). The only per-person step left is each teammate creating their own Routine(s) + API token in their claude.ai and sending the **routine id+token**, which fills the `routines` block of their file on the relay (hot-reloaded, no restart; template in `.claude/integrations/slack/relay/person.example.json`), mirrored as a Secure Note in the LastPass PMM folder. They do NOT connect personal Slack/Jira MCP connectors — Slack replies go through the relay bot, Jira posts go through the shared REST token in the environment. Their routine runs in the **shared team environment** (network `Full`, already live — see next item).
