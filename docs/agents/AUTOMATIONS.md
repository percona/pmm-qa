...
- [ ] Connector MCP tools (Atlassian Rovo, Slack, Claude Code Remote) allowlisted so interactive web sessions don't prompt on every call — **status: inconclusive, needs a fresh-session test**. What one contaminated interactive session established: (1) `mcp__github` in `permissions.allow` genuinely suppresses prompts (that server is project-configured, not a connector); (2) settings.json permission/hook edits **hot-reload mid-session** — no restart needed; (3) for the connector servers, none of the three candidate rule spellings could be cleanly proven or disproven — the display name (`mcp__Atlassian_Rovo`), the documented connector form `mcp__claude_ai_<server>` ([docs](https://code.claude.com/docs/en/permissions#mcp-permission-rules): "Connector tools appear as `mcp__claude_ai_<server>__<tool>`"), and the space-preserving variant (`mcp__claude_ai_Atlassian Rovo`) — because the web UI **remembers per-tool approve/deny choices server-side** (an early manual deny of `atlassianUserInfo` auto-denied all later calls in ~0.7s; an early approval of a Slack read let later calls pass), and those remembered choices shadow any rule effect, invisibly to the agent. An agent inside a session **cannot see whether a prompt fired** — approved-prompt and no-prompt look identical — so verification needs a human watching. **Test protocol** (fresh session, zero clicks): check out this branch, run one read call each against Atlassian/Slack/CCR, the human notes which ones visibly prompt (deny them all), and timing (`date` before/after) corroborates. settings.json currently carries all candidate spellings for the test. **Worth knowing regardless**: the live Routines already post to Jira from fired sessions (their connector access comes from the grant attached at `create_trigger` time), so interactive-session prompting has NOT been observed to stall an actual Routine run — this item is about interactive/web sessions and about not carrying misleading no-op entries in settings.json. Also: if the org sets a connector tool to `ask` in admin settings, allow rules are overridden by design.
