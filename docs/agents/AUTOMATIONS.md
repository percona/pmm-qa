...
Delivery reuses the **relay bot**, so no MCP connector is needed in the Routine (which sidesteps the connector-in-routine limits): the run `curl`s the composed digest to the relay's `POST /slack/announce` (gated by `RELAY_KEY` + the caller's verified `X-GitHub-Token`), which posts it as the `@pmm-ai` bot to a channel it's been invited to. Daily, weekdays, **no @-mentions**.

## Secrets architecture — relay-brokered (built; prod cutover pending)

> **Status (2026-08-12):** the broker (`/<service>/<action>`) is implemented and **validated end-to-end on a staging relay** (assert mode, roster = `travagliad`): `/linode/provision` → `{ip, exec_token(40), exec_cert_pem}`; `/linode/destroy` → `ok` (the previously-blocked delete now works, via the events-read token **and** the `skip_instance_*_poll` fix); `/jira/read` → the ticket; and the gate rejects a missing `X-Actor` (401), a non-roster actor (403), and a wrong `RELAY_KEY` (403). The unspoofable `verify` mode is coded and needs a real GitHub token to exercise (see modes below). Remaining human step: prod cutover — set `RELAY_KEY`/`RELAY_GH_ALLOW`/`RELAY_IDENTITY_MODE` in the qa-linode env, deploy the current relay, and remove `LINODE_TOKEN`/`JIRA_*` from the shared env. The unspoofable `verify` mode is coded and needs a real GitHub token to exercise (see modes below). Prod cutover (set `RELAY_KEY`/`RELAY_GH_ALLOW`/`RELAY_IDENTITY_MODE` in the qa-linode env, remove `LINODE_TOKEN`/`JIRA_*` from the shared env) is the remaining human step.
...
**Identity is an access control, not just a log.** Every broker call carries the caller's GitHub identity, checked against **`RELAY_GH_ALLOW`** (the team roster) — a leaked `RELAY_KEY` alone doesn't name a valid member. The verified login is the audit `by`; the old self-reported email is gone.

The relay has **two identity modes** because of a platform constraint — a Claude Code session's GitHub token is a *proxy-brokered placeholder* (`prox…`) that only authenticates **through** the egress proxy, so a standalone relay calling GitHub `/user` with it gets `github_auth_failed`, and write-based proofs (gists) are blocked by the proxy:

- **`assert` (default, today's env):** the hard gate is `RELAY_KEY` = **shared-env membership** (Anthropic-admin-controlled). The caller sends `X-Actor` — its login from `gh api user`, which the *proxy* really verified — and the relay roster-checks and logs it. This is **not cryptographically unspoofable at the relay boundary** (an attacker already inside the env could forge `X-Actor`), but a bare leaked `RELAY_KEY` still needs a valid roster login, and every action is attributed.
- **`verify` (unspoofable):** the relay validates `X-GitHub-Token` against GitHub `/user` directly. Turn this on wherever callers hold a **real** (non-proxy) GitHub token — a non-proxy deploy, or a per-user PAT. The code path is built and tested; it just needs a real token to verify against.

**To close the spoofability gap** without per-user PATs: give the relay its own read-only GitHub token and add a one-time **push-proof** handshake (caller pushes a relay-issued nonce to a `pmm-qa` branch — `git push` *is* allowed through the proxy and is attributed to the real GitHub user; the relay reads the real pusher via the events API and issues a short-lived session token). Designed, not yet built — it's the recommended next step.
...
- **Linode: fully covered by `/linode/provision` + `/linode/destroy`.** `LINODE_TOKEN` is used *only* by `up.sh` (create) and `down.sh` (destroy); `run.sh`, `extend.sh`, `sync.sh` have **zero** `LINODE_TOKEN` refs and drive the VM through the per-run `exec_token` that provision returns. So `run.sh` needs that `exec_token`, never the account token.
  - **Token stays minimal.** The terraform Linode provider polls `/v4/account/events` to confirm a delete, which 401s a token without `events:read`. Rather than widen the token, `versions.tf` sets `skip_instance_ready_poll` + `skip_instance_delete_poll` — the instance still creates/deletes, terraform just doesn't block on the event — so the relay token needs only **Linodes + Firewalls R/W**.
  ...
  - **The `SessionEnd` cleanup hook is rewired (done).** `session-end-cleanup.sh` now POSTs `/linode/destroy` (with `RELAY_KEY` + `X-GitHub-Token`) for relay-brokered runs, keeps the legacy `down.sh` path for local state, and **skips any run with a `keep-alive` marker**. It stays in `settings.json`; only its teardown call changed. The on-box self-destruct timer remains the actual guarantee.
- **Jira: fully covered by `/jira/<action>`.** read + comment (Developers-only, forced) + field + transitions + transition + attach — the whole set the `jira` skill uses; create/delete are refused by construction.
...
- [x] **`LINODE_TOKEN` delete-path 401** — the provider's instance-delete lists `/v4/account/events`, which 401s a token without `events:read`. Fixed *without widening the token* by `skip_instance_ready_poll` + `skip_instance_delete_poll` in `versions.tf`, so the token stays Linodes+Firewalls R/W (2026-08-12). (A separate testing token *with* `events:read_only` also works — either path is fine.)
...
- [ ] Add `RELAY_KEY` (and `RELAY_GH_ALLOW` with the team's GitHub logins) to the relay `.env`, and set `RELAY_KEY` in the qa-linode environment, then redeploy the relay so `POST /slack/announce` is live. The Routine authenticates with `RELAY_KEY` + its `X-GitHub-Token`.
