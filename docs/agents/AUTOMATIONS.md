...
**Migration lessons are outside this loop entirely.** A lesson whose target is a CodeceptJS-migration file — anything under `.claude/skills/codeceptjs-migration/`, the three `pmm-migration-*` agents, or `migration-phase-observe.sh` — captured on the migration control branch goes to `.claude/skill-lessons-migration/` and is committed on that branch. Those branch versions differ from `main`, so promoting one would open a PR against a different file and then let the implement step edit it unattended. The publisher never reads that directory, and nothing applies those lessons automatically: the user reviews them and asks, and the edit is committed on the control branch. The separate directory is what makes the exclusion structural rather than a rule the publisher has to remember.
