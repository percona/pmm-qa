...
> **Status (2026-08-12):** the broker (`/<service>/<action>`) is implemented and validated on a staging relay — endpoint consolidation, the `skip_instance_*_poll` token fix, and the identity gate's **deny** paths all pass (missing/bad `X-GitHub-Token` → 401, wrong `RELAY_KEY` → 403). **Open blocker on the identity allow-path:** the GitHub token in a Claude Code session is a *proxy-brokered* placeholder (`prox…`) that only authenticates *through* the egress proxy; a standalone relay calling GitHub `/user` with it gets `github_auth_failed`, and write-based proofs (gists) are blocked by the proxy too. So the relay cannot verify a caller's GitHub identity in this environment. Pending a decision on the identity mechanism (env-membership-as-gate vs. a push-based unspoofable proof vs. real per-user tokens). Prod cutover (set `RELAY_KEY`/`RELAY_GH_ALLOW`, remove `LINODE_TOKEN`/`JIRA_*` from the shared env) waits on that.
...
**Identity is an access control, not just a log — and it's unspoofable.** Every broker call carries the caller's **GitHub token** (`X-GitHub-Token`). The relay asks GitHub `/user` who that token belongs to (the caller can't fake it — GitHub is the authority), then checks the login against **`RELAY_GH_ALLOW`** (the team roster). So a leaked `RELAY_KEY` alone is **denied** — you need the key *and* a verified team GitHub identity. The verified login becomes the audit `by`; the old self-reported email is gone (it was spoofable). Empty allowlist = any *verified* GitHub user (identity still proven); set it to lock down to the team.
