...
- [ ] PMM AI Slack app + Router Routine — **candidate design: a small Cloudflare Worker relay** as the single deterministic entry point: Slack Events API (`app_mention`) AND the one Jira Automation rule both POST to the Worker; it maps the initiator (Slack user ID / Jira `{{initiator.accountId}}`) to that person's Routine and fires `/fire` with the per-person token from its secret store; unmapped users fall back to the shared Test Runner (owner's identity). Solves this item and per-person routing with one component, no tokens in this repo or in env vars. Slack Workflow Builder webhook step was checked and isn't available on the current plan/permissions.
