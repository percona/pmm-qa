...
- [ ] Routine runs stall on connector approval prompts despite attached connectors — **confirmed platform bug, [claude-code#61015](https://github.com/anthropics/claude-code/issues/61015)**: Routine runs are documented as prompt-free for included connectors ([Routines docs](https://code.claude.com/docs/en/routines): "no approval prompts during a run … including writes, without asking for permission"), but a regression (~2026-05-20) makes runs demand approval anyway on custom/org connectors — first-party connectors were unaffected, and Atlassian Rovo (Atlassian's remote MCP registered as an org connector) is in the affected class. Observed live: a Test Runner run on PMM-15188 stalled on "This connector call requires your approval to proceed" for `getAccessibleAtlassianResources` **with the connector attached**. The issue is closed as fixed upstream but still reproduces here (2026-08-06). **Actions**: (1) comment on #61015 with this repro (Routine trigger ID, connector = Atlassian Rovo, date, prompt text) and ask for a reopen — only Anthropic can fix the grant enforcement; (2) until fixed, unattended runs must avoid the connector for Jira: the `jira` skill now has a **REST fallback** (`curl` + `JIRA_EMAIL`/`JIRA_API_TOKEN`) with the same Developers-only visibility rules — Bash has no approval gate, so Routines complete without a human; (3) add `JIRA_EMAIL` + `JIRA_API_TOKEN` to the cloud environment(s) the Routines use (same caveats as `LINODE_TOKEN` above: plaintext-visible to environment users, so scope the token to a service account if possible), and confirm `perconadev.atlassian.net` is reachable under the environment's network policy. Still worth verifying the connector lists at [claude.ai/code/routines](https://claude.ai/code/routines) (only visible there; `list_triggers` doesn't return them) so the connector path starts working the moment the platform bug is actually fixed.
...
- [x] Connector permission prompts — **understood; resolved as "works as designed, fix belongs in Routine config, not this repo"**. Established across one long interactive web session (with human-observed prompts, since an agent cannot see whether a prompt fired — approved-prompt and no-prompt look identical from inside):
  - **Interactive web sessions**: connector tool prompts ("This connector call requires your approval to proceed") are **enforced by the claude.ai host layer, not Claude Code's permission engine** — connectors arrive host-provisioned ([mcp docs](https://code.claude.com/docs/en/mcp#use-mcp-servers-from-claude-ai): "In Claude Code on the web sessions, claude.ai connectors are provisioned by the remote host … Manage which connectors a cloud session can use from your claude.ai organization settings"). Nothing in this repo can suppress them: `permissions.allow` entries in all three plausible spellings (display name `mcp__Atlassian_Rovo`, documented `mcp__claude_ai_<server>`, space-preserving) and a PreToolUse hook returning `permissionDecision: "allow"` (valid JSON, matcher verified against real tool names) were all tested — every connector call still prompted. The web UI remembers per-tool approve/deny choices server-side within a session, which contaminates repeat testing. Connector server names are also **not stable across sessions** (`Atlassian_Rovo` in one, `Atlassian-Rovo` in another), so name-based rules are fragile even where they apply.
  - **Routine-fired runs**: prompts are governed by the **Routine's connector list**, not by this repo ([routines docs](https://code.claude.com/docs/en/routines): "Routines run autonomously … no approval prompts during a run"; included connectors are usable "including writes, without asking for permission"). A prompt during a run = connector missing from the grant → fix in the web UI (see the connector-attachment item above).
  - **What settings.json still does**: `mcp__github` allowlisting genuinely works (project-provisioned server, normal permission engine); the `mcp__claude_ai_<server>` entries are kept because that's the documented rule form for claude.ai connectors in **local CLI sessions** — they are inert in web sessions. Settings/hook edits hot-reload mid-session; no restart needed.
...
- [ ] **Cross-org GitHub API access to `Percona-Lab/*` — confirmed BLOCKED in a `percona/*`-seeded session; needs a session-seeding change (not admin-only).** Re-verified from a fresh Claude Code web session on `percona/pmm-qa`: `gh api repos/Percona-Lab/pmm-submodules` and `…/jenkins-pipelines` both `403` with "access to this repository is not enabled for this session"; the `github` MCP tools return "Access denied: repository … is not configured for this session"; even unauthenticated `api.github.com` `403`s through the proxy. **Root cause** (not a `gh auth` sandbox quirk): `add_repo` with `access:"push"` is refused — `cross-tier adds are not supported in v1: session already has repos from owner(s) [percona]`. A session holds credentialed GitHub API access to **one owner tier only**, and this session is seeded with `percona/*`. What *does* work cross-org: **anonymous git read** — `add_repo` (`access:"read"`) confirms the public repo is already readable via the git proxy, and `git ls-remote` / shallow `git clone` succeed (a bare `git clone …pmm-submodules` is still blocked by the PreToolUse hook by design). **Fixes:** (a) fire the Investigator/FB-Reporter Routine in a session **seeded with `Percona-Lab/pmm-submodules` as the initial source** so `gh pr checks` / `gh api` work there; and/or (b) pass FB-Tests status (run URL + conclusion) into the Routine via the `notify-investigator.yml` trigger payload (line below) so no cross-org API read is needed for the common path; and/or (c) read files/diffs via anonymous git read, which needs no attach. Note (b) and seeding on `Percona-Lab` conflict: a `Percona-Lab`-seeded session can't credential-attach `percona/pmm-qa` either (same v1 rule, reversed), so a Routine that must open a **fix PR in `percona/pmm-qa`** should stay `percona/*`-seeded and lean on (b)/(c). Separately confirm the Claude GitHub App is installed on the `Percona-Lab` org before relying on any credentialed cross-org path if v1 lifts this restriction — that's the one genuinely admin-only piece (admin grants access at `https://claude.ai/admin-settings/claude-in-slack`). Encouragingly, `list_repos` from a live web session reports `can_push: true` for both `Percona-Lab/pmm-submodules` and `Percona-Lab/jenkins-pipelines` — the connected GitHub account already has access to the org, so the v1 refusal is purely the per-session owner-tier rule, and fix (a) (seeding a session with the `Percona-Lab` repo as initial source) should work without any admin action.
- [ ] **Multi-repo sessions across orgs — the intended cross-org path; needs one live verification.** Findings that make this the plan of record: (1) multi-repo is official ([web quickstart](https://code.claude.com/docs/en/web-quickstart): "You can add multiple repositories to work across them in one session"; [routines docs](https://code.claude.com/docs/en/routines): "Add one or more GitHub repositories"), with **no documented same-owner restriction at creation** — the v1 cross-tier refusal is specific to the mid-session `add_repo` flow; (2) the GitHub proxy scopes API access to "repositories **attached to the session**" ([cloud-environments docs](https://code.claude.com/docs/en/cloud-environments)), so selecting both repos at creation should light up `gh api`/`gh run rerun` for both, including `Percona-Lab/pmm-submodules` CI re-runs; (3) **a personal-token workaround is impossible**: verified live that the proxy replaces whatever token `gh` sends (a deliberately bogus `GH_TOKEN` still succeeded against the in-scope repo) — session scope is decided by the proxy, never by your credential, so a `Percona-Lab` PAT in an env var does nothing for `api.github.com`. **Verified live 2026-08-06** (multi-repo test session, `percona/pmm-qa` + `Percona-Lab/pmm-submodules`): **cross-org API scope PASSES** — both clones land side by side (`/home/user/pmm-qa`, `/home/user/pmm-submodules`), `gh api` succeeds against both repos, and `gh run list`/`gh run rerun <id> --failed -R Percona-Lab/pmm-submodules` is available (the CI re-run capability this item exists for). **But hooks/permissions FAIL to load in that shape**: the session roots at `/home/user` — the *parent* of the clones, no primary repo at all — with `$CLAUDE_PROJECT_DIR` unset, so no project `.claude/settings.json` is loaded from anywhere (repo `CLAUDE.md`/agents/skills DO load; it's specifically settings.json that's skipped — no SessionStart tooling, no clone guard, no permission allowlist). **Fix — user-scope settings planted by the environment setup script** (configured in the web UI only — deliberately not shipped as a repo file, since it can never run from the repo; this snippet is the record of what the environment is set to):
  ...
  It places the committed settings at `/root/.claude/settings.json` (user scope loads regardless of working directory): local pmm-qa clone wins when present, otherwise it fetches `.claude/settings.json` from `percona/pmm-qa@main` via `raw.githubusercontent.com` (in the default Trusted allowlist; fetch verified live) — single source of truth, no duplicated JSON. The hooks' self-locating `/home/user/pmm-qa` fallback resolves in the verified layout, and in sessions without a pmm-qa clone the wrappers find nothing and exit 0 (harmless). **Mechanism fully verified 2026-08-06, pre-merge, in a throwaway test environment**: (stage 1) user-scope `/root/.claude/settings.json` IS honored by cloud sessions — copying the settings there mid-session immediately activated the PreToolUse clone guard (hot-reload); (stage 2) with the script as a test environment's setup script fetching from this branch via `raw.githubusercontent.com`, a fresh multi-repo session had the settings planted, the SessionStart hook fired from user scope at startup (gh/terraform/ffmpeg installed before the first turn), the clone guard blocked, and `gh api repos/Percona-Lab/pmm-submodules` succeeded — all four checks passed with no permission prompts. **Caveats**: (1) setup-script results are snapshot-cached ~7 days ([docs](https://code.claude.com/docs/en/cloud-environments#environment-caching)), so settings changes on main reach the environment on the next cache rebuild — force it early by touching the environment's setup script; (2) this applies the allowlist environment-wide, to every session in that environment — intended for the QA-automation environment, don't reuse for unrelated ones; (3) pre-cloning extra repos in the setup script was considered and rejected: it yields stale (~7-day) file copies only — GitHub **API** scope ("gh run rerun", pushes, PRs) is fixed by the repos attached at session creation, and per the docs "a setup script that downloads release assets from an unattached repository gets a 403", so Routine repo lists remain the only lever for API access. **Remaining to do (post-merge)**: (1) set the real QA environment's setup script to the snippet above and archive the test environment; (2) add `Percona-Lab/pmm-submodules` to the Investigator and FB Reporter Routines' repository lists at [claude.ai/code/routines](https://claude.ai/code/routines); (3) optional: file an upstream issue — multi-repo sessions loading `CLAUDE.md`/skills from repo `.claude/` but silently skipping `settings.json` is an inconsistency worth reporting.
