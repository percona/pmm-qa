...
- **Linode: fully covered by `/provision` + `/destroy`.** `LINODE_TOKEN` is used *only* by `up.sh` (create) and `down.sh` (destroy) — 3 refs each; `run.sh`, `extend.sh`, `sync.sh` have **zero** `LINODE_TOKEN` refs and drive the VM through the per-run `exec_token` that `/provision` returns. So `run.sh` needs that `exec_token`, never the account token.
  - **Keep-alive is preserved.** "Provision and leave it up for me" still works: `/provision` takes `ttl_hours` (default 24h; any requested window honored — no fixed cap), the agent simply skips `/destroy`, and the on-box self-destruct timer still cleans up eventually. `extend.sh` pushes that timer back via the `exec_token`, no `LINODE_TOKEN` involved. The agent returns the PMM URL + generated admin password as today.
  - **The `SessionEnd` cleanup hook needs rewiring.** `session-end-cleanup.sh` (best-effort teardown of this session's own runs) calls `down.sh`, so it uses `LINODE_TOKEN` and exits early without it. It gets pointed at the relay's `/destroy` (with `RELAY_KEY`) instead — it **stays in `settings.json`**, only its teardown call changes. The on-box self-destruct timer remains the actual guarantee, as the hook's own comment says.
...
- [ ] **Widen the two agent credentials** — add the `events:read_only` scope to the `LINODE_TOKEN` PAT (the provider's instance-delete path lists `/v4/account/events`, so `down.sh` 401s without it) and grant the GitHub App installation on `percona/*` **Actions: Read and write** (`rerun-failed-jobs` answers `X-Accepted-Github-Permissions: actions=write`), both hit live on 2026-08-12 in [run 31549464587](https://github.com/percona/pmm-qa/actions/runs/31549464587).
