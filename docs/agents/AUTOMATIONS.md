...
| [pr-maintainer](../../.claude/agents/pr-maintainer.md) | Every open `percona/pmm-qa` PR | Scheduled — a daily weekday Routine | Reads each open PR and sorts it into ready / unblocked / needs-review / blocked-on-upstream / needs-work / needs-a-human, maintains a `blocked` label, and posts a **PR Digest** to Slack `#qa-automation` via the relay bot | Merge, close, approve, or edit any PR — the `blocked` label is the only thing it changes |
...
It **reads** each PR (not a keyword scan) and sorts it (first match wins; bias to "needs a human"): 🔓 **Unblocked** (carries the `blocked` label but the upstream it waited on has now merged → promote/re-verify; the agent removes the label), ⏳ **Blocked on upstream** (the agent *judges* it's waiting on a not-yet-merged upstream PR and adds a `blocked` label), 🔧 **Needs work** (open review threads — whoever opened them — or changes requested), ✅ **Ready to merge** (approved, no open threads, not blocked), 👀 **Needs review** (waiting on a reviewer), and ❓ **Needs a human** (nothing fits cleanly). **CI checks are deliberately ignored** — an approved PR is ready regardless. The `blocked` label (add/remove) is the only thing it ever writes; it never merges, closes, approves, or edits.
