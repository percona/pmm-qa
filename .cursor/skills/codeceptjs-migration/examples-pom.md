...
export default class SettingsPage extends BasePage {
  url = '/pmm-ui/settings';
  successPopUpMessage = 'Settings saved';
  ...
  messages = {
    successPopUp: this.page.locator('[role="alert"], [role="status"]'),
  };
...
5. **Behavior preserved**: `applyChanges()` performs the click only; the migrated test asserts `messages.successPopUp` inline with `expect(...)`.
