...
export const installPmmClient = async (containerName: string) => {
  ...
  await executeCommand(`docker exec ${containerName} wget https://repo.percona.com/apt/percona-release_latest.$(lsb_release -sc)_all.deb`);
...
export const connectPmmClient = async (containerName:string, serverAddress: string, username: string = 'admin', password: string = 'admin') => {
  await executeCommand(`docker exec ${containerName} pmm-agent setup --config-file=/usr/local/percona/pmm2/config/pmm-agent.yaml --server-address=${serverAddress} --server-insecure-tls --server-username=${username} --server-password=${password}`);
}
