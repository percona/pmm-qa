...
docker compose -f docker-compose-sharded-with-pmm.yaml exec -T mongos pmm-admin add mongodb --enable-all-collectors --agent-password=mypass --cluster=sharded --environment=mongo-sharded-dev --username=${pmm_user} --password=${pmm_pass} mongos_${random_number} 127.0.0.1:27017
...
if [ $tests != "no" ]; then
    echo "running tests"
    docker compose -f docker-compose-sharded-with-pmm.yaml run test pytest -s -x --verbose test.py
    docker compose -f docker-compose-sharded-with-pmm.yaml run test chmod -R 777 .
    else
    echo "skipping tests"
fi
cleanup=${CLEANUP:-yes}
