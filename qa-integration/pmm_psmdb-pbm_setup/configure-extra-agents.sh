...
do
    ...
    if [[ $mongo_setup_type == "psa" && $node == "rs203"  ]]; then
      docker compose -f docker-compose-rs.yaml exec -T $node pmm-admin add mongodb --enable-all-collectors --query-source="${mongo_query_source}" --agent-password=mypass --cluster=replicaset --replication-set=rs1 --host=${node} --port=27017 ${node}${gssapi_service_name_part}_${random_number}
    else
      docker compose -f docker-compose-rs.yaml exec -T $node pmm-admin add mongodb --enable-all-collectors --query-source="${mongo_query_source}" --agent-password=mypass --cluster=replicaset "${client_credentials_flags[@]}" --host=${node} --port=27017 ${node}${gssapi_service_name_part}_${random_number}
    fi
done
