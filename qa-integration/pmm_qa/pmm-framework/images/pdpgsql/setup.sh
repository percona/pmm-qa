#!/usr/bin/env bash
#
# images/pdpgsql/setup.sh -- PDPGSQL and SSL_PDPGSQL, on the prebaked pdpgsql and ssl-pdpgsql images.
...
pdpgsql_workload() {
  ...
  case $setup_type in
    ...
    replication)
      ...
      must docker cp "$data/load_pgsql.sql" "$primary:/load_pgsql.sql"
