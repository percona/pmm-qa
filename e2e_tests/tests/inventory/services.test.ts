...
pmmTest.beforeEach(async ({ grafanaHelper, servicesPage, page }) => {
  await grafanaHelper.authorize();
  await page.goto(servicesPage.url);
});

pmmTest('PMM-T2159 - Verify MongoDB RTA Agent displayed in Inventory UI @rta', async ({ api, servicesPage, agentsPage }) => {
