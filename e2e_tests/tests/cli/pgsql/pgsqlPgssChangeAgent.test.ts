...
pmmTest.describe('Tests to verify pmm-admin inventory change agent functionality', () => {
  ...
  pmmTest(
    'PMM-T9991 - Verfiy Change agent username and password @pgss-pmm-integration',
  ...
  pmmTest(
    'PMM-T9992 - Verfiy Change agent custom labels @pgss-pmm-integration',
  ...
  pmmTest(
    'PMM-T9993 - Verify Change agent log level @pgss-pmm-integration',
    async ({ agentsPage, cliHelper, grafanaHelper, page }) => {
      const commands = [
  ...
  pmmTest(
    'PMM-T9993 - Verify Change agent debug, trace and json @pgss-pmm-integration',
    async ({ cliHelper }) => {
      const commands = [
  ...
  pmmTest(
    'PMM-T9994 - Verify Change agent tls @pgss-pmm-integration',
    async ({ cliHelper, grafanaHelper, page, servicesPage }) => {
      ...
      commands = [
        ...
        `docker exec ${containerName} pmm-admin inventory change agent qan-postgresql-pgstatements-agent ${pgStatStatementId} --tls-cert-file=/certs/client.crt --tls-key-file=/certs/client.key --tls-ca-file=/certs/ca-certs.pem --tls --tls-skip-verify`,
      ];
  ...
  pmmTest(
    'PMM-T9995 - Verify Change agent enable true/false @pgss-pmm-integration',
    async ({ cliHelper, page }) => {
      ...
      for (const enableCommand of enableCommands) {
        const commands = [
          ...
          `docker exec ${containerName} pmm-admin inventory change agent qan-postgresql-pgstatements-agent ${pgStatStatementId} ${enableCommand.command}`,
        ];
  ...
  pmmTest(
    'PMM-T9996 - Verify Change agent agent password @pgss-pmm-integration',
    async ({ cliHelper, page }) => {
      const commands = [
        ...
        `docker exec ${containerName} pmm-admin inventory change agent qan-postgresql-pgstatements-agent ${pgStatStatementId} --agent-password=${pgExporterPassword}`,
      ];
  ...
  pmmTest(
    'PMM-T9993 - Verify Change agent expose exporter @pgss-pmm-integration',
    async ({ cliHelper, page }) => {
  ...
  pmmTest(
    'PMM-T9993 - Verify Change agent push metrics @pgss-pmm-integration',
    async ({ cliHelper, page }) => {
      pgExporterPort = cliHelper
        .execSilent(
          `docker exec ${containerName} pmm-admin list | grep ${pgExporterId} | awk -F' ' '{print $6}'`,
  ...
  pmmTest(
    'PMM-T9993 - Verify Change agent disable collectors @pgss-pmm-integration',
    async ({ api, cliHelper }) => {
  ...
  pmmTest(
    'PMM-T9993 - Verify Change agent max exporter connections @pgss-pmm-integration',
    async ({ cliHelper, page }) => {
      await cliHelper
        .execSilent(
  ...
  pmmTest(
    'PMM-T1017 - Verify Change agent auto-discovery limit @pgss-pmm-integration',
    async ({ api, cliHelper }) => {
  ...
  pmmTest(
    'PMM-T9993 - Verify Change agent pmm agent listen port @pgss-pmm-integration',
    async ({ cliHelper, page }) => {
      let commands = [
      ...
      commands = [
        ...
        `docker exec ${containerName} pmm-admin inventory change agent qan-postgresql-pgstatements-agent ${pgStatStatementId} --pmm-agent-listen-port=7778`,
      ];
      commands.forEach((command) => cliHelper.execSilent(command).assertSuccess());
    },
  );
  ...
  pmmTest(
    'PMM-T99103 - Verify Change agent server url and server insecure tls @pgss-pmm-integration',
    async ({ cliHelper }) => {
      ...
      let commands = [
        ...
        `docker exec ${containerName} pmm-admin inventory change agent qan-postgresql-pgstatements-agent ${pgStatStatementId} --server-url=${serverUrl}`,
      ];
      ...
      commands = [
        ...
        `docker exec ${containerName} pmm-admin inventory change agent qan-postgresql-pgstatements-agent ${pgStatStatementId} --server-url=${serverUrl} --server-insecure-tls`,
      ];
