...
pmmTest.describe('Tests to verify pmm-admin inventory change agent functionality', () => {
  ...
  pmmTest(
    'PMM-T9993 - Verify Change agent debug, trace and json @ps-slowlog-integration',
    async ({ cliHelper }) => {
      const commands = [
  ...
  pmmTest(
    'PMM-T9996 - Verify Change agent agent password @ps-slowlog-integration',
    async ({ cliHelper, page }) => {
      cliHelper.execSilent(
  ...
  pmmTest(
    'PMM-T9993 - Verify Change agent expose exporter @ps-slowlog-integration',
    async ({ cliHelper, page }) => {
  ...
  pmmTest(
    'PMM-T9993 - Verify Change agent push metrics @ps-slowlog-integration',
    async ({ cliHelper, page }) => {
      pgExporterPort = cliHelper
        .execSilent(
          `docker exec ${containerName} pmm-admin list | grep ${mysqldExporterId} | awk -F' ' '{print $6}'`,
  ...
  pmmTest(
    'PMM-T1016 - Verify Change agent comments parsing @ps-slowlog-integration',
    async ({ api, cliHelper }) => {
  ...
  pmmTest(
    ...
    async ({ api, cliHelper }) => {
      ...
      await cliHelper
        .execSilent(
          `docker exec ${containerName} pmm-admin inventory change agent qan-mysql-slowlog-agent ${mysqldSlowlogAgentId} --max-query-length=${maxQueryLength}`,
        )
        .assertSuccess()
  ...
  pmmTest(
    'PMM-T9993 - Verify Change agent pmm agent listen port @ps-slowlog-integration',
    async ({ cliHelper }) => {
      let commands = [
  ...
  pmmTest(
    'PMM-T99103 - Verify Change agent server url and server insecure tls @ps-slowlog-integration',
    async ({ cliHelper }) => {
      ...
      let commands = [
        ...
        `docker exec ${containerName} pmm-admin inventory change agent qan-mysql-slowlog-agent ${mysqldSlowlogAgentId} --server-url=${serverUrl}`,
      ];
      ...
      commands = [
        ...
        `docker exec ${containerName} pmm-admin inventory change agent qan-mysql-slowlog-agent ${mysqldSlowlogAgentId} --server-url=${serverUrl} --server-insecure-tls`,
      ];
