...
Data(panels).Scenario(
  ...
  async ({
    ...
    I.click(dashboardPage.fields.filterDropdownOptionLocator(nodeNames[0]));
    I.click(dashboardPage.fields.filterDropdownOptionLocator(nodeNames[1]));
    I.pressKey('Escape');
    I.wait(2);
    ...
    const expectedNodeName = dashboardType === 'singleNode'
      ? nodeNames.sort()[0]
      : await dashboardPage.getSelectedFilterValues('Node Name');
    ...
    if (dashboardType === 'singleNode') {
      await I.waitForText(expectedNodeName, 20, dashboardPage.fields.filterSelectedValues('Node Name'));
    } else {
      const values = await dashboardPage.getSelectedFilterValues('Node Name');

      await assert.ok(values.join('') === expectedNodeName.join(''), `Expected ${expectedNodeName.join('')} to be in ${values.join('')}`);
    }
