...
BeforeSuite(async ({ I }) => {
  ...
  assert.ok(
    dockerCheck.includes(containerName),
    'rs101 docker container should exist. please run pmm-framework with --database psmdb',
  );
});
...
Scenario('PMM-T1241 - Verify add mongoDB service with "+" in user password @mongodb-exporter', async ({ I, grafanaAPI }) => {
  await I.say(
    await I.verifyCommand(
      `docker exec ${containerName} pmm-admin add mongodb ${clientCredentialsFlags} --host=${containerName} --service-name=${mongodb_service_name}`,
    ),
  );
