...
const verifyEncryptionRotation = async (container) => {
  ...
  assert.ok(r.includes('Added new primary encryption key'), 'Failed to add new encryption key');
  assert.ok(r.includes('Restarting PMM Server'), 'Failed to restart PMM Server');
  assert.ok(r.includes('re-encrypted with the new key'), 'Failed to re-encrypt stored secrets');
};
