...
Data(filters).Scenario(
  'PMM-T1054 + PMM-T1055 - Verify the "Command type" filter for Postgres @qan @nightly-pdpgsql',
  async ({
...
Scenario(
  'PMM-T175 - Verify user is able to apply filter that has dots in label @qan @nightly-pdpgsql',
...
Scenario(
  'PMM-T172 - Verify that selecting a filter updates the table data and URL @qan @nightly-psmdb',
  async ({ I, queryAnalyticsPage }) => {
...
Scenario(
  'PMM-T126 - Verify user is able to Reset All filters @qan @nightly-psmdb',
  async ({ I, queryAnalyticsPage }) => {
    const environmentName1 = 'pxc-dev';
    const environmentName2 = 'rs101';
...
Scenario(
  'PMM-T125 - Verify user is able to Show only selected filter values and Show All filter values @qan @nightly-psmdb',
  async ({ I, queryAnalyticsPage }) => {
    const environmentName1 = 'pxc-dev';
    const environmentName2 = 'rs101';
...
Scenario(
  'PMM-T123 - Verify User is able to search for DB types, Env and Cluster @qan @nightly-psmdb',
  async ({ I, queryAnalyticsPage }) => {
    const filters = [
      'Environment',
      'Cluster',
...
Scenario(
  'PMM-T191 - Verify Reset All and Show Selected filters @qan @nightly-psmdb',
  async ({ I, queryAnalyticsPage }) => {
    const environmentName1 = 'pxc-dev';
    const environmentName2 = 'rs101';
...
Scenario('PMM-T190 - Verify user is able to see n/a filter @qan @nightly-percona-server', async ({ I, queryAnalyticsPage }) => {
...
Scenario(
  'PMM-T390 - Verify that we show info message when empty result is returned @qan @nightly-percona-server',
  async ({
...
Scenario(
  'PMM-T221 - Verify that all filter options are always visible (but some disabled) after selecting an item and % value is changed @qan @nightly-percona-server',
  async ({
    I, adminPage, queryAnalyticsPage,
  }) => {
...
*/
Scenario('PMM-T437 - Verify short-cut navigation for n/a items @qan @nightly-psmdb', async ({ I, queryAnalyticsPage }) => {
  queryAnalyticsPage.waitForLoaded();
...
Scenario('PMM-T2032 - Verify there is no name with brackets in Plan Summary in QAN @qan @nightly-psmdb', async ({ I, queryAnalyticsPage, adminPage }) => {
  queryAnalyticsPage.waitForLoaded();
