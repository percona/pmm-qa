...
Scenario('PMM-T269 - Verify QAN UI Elements are displayed @qan', async ({ I, queryAnalyticsPage }) => {
  ...
  I.wait(3);
  I.waitForElement(queryAnalyticsPage.filters.fields.filterCheckBoxesInGroup('Service Name'), 10);
  ...
  await queryAnalyticsPage.filters.selectContainFilterInGroup('pmm-server', 'Service Name');
...
Scenario(
  ...
  async ({ I, adminPage, queryAnalyticsPage }) => {
    ...
    let detailsValue = await I.grabTextFrom(
      queryAnalyticsPage.queryDetails.elements.metricsCellDetailValue('Query Count', 2),
      30,
    );

    assert.ok(
      overviewValue === detailsValue,
      `Query Count value in Overview and Detail should match. Overview:'${overviewValue}'!=Detail:'${detailsValue}'`,
    );
    ...
    assert.ok(
      overviewValue === detailsValue,
      `Query Time value in Overview and Detail should match. Overview:'${overviewValue}'!=Detail:'${detailsValue}'`,
    );
  },
);
...
Scenario('PMM-T188 - Verify dashboard refresh @qan', async ({ dashboardPage, adminPage, queryAnalyticsPage }) => {
  ...
  await queryAnalyticsPage.data.verifySearchByValue('postgres');
  ...
  async ({ I, queryAnalyticsPage, credentials }) => {
    ...
    if (psVersion > 8.0) {
      await I.verifyCommand(
        `docker exec ${testContainerName} mysql -h 127.0.0.1 -u ${root.username} -p${root.password} --port 3306 -e "SET MAX_EXECUTION_TIME = 1000;"`,
      );
    } else {
