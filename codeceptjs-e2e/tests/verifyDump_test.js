...
BeforeSuite(async ({ I, codeceptjsConfig }) => {
  await I.verifyCommand(
    `docker run --name sftp-server --network pmm-qa -v ${hostVolume}:/home/foo${sftp.directory} -p 2222:22 -d atmoz/sftp ${sftp.username}:${sftp.password}`,
  );
  await I.wait(30);
...
Scenario('PMM-T1835 - Create Dump Archive and Verify its successful in UI @dump', async ({ dumpAPI, dumpPage }) => {
...
Scenario('PMM-T1835 - Verify Edit Buttons are Enabled for Dump @dump', async ({ dumpAPI, dumpPage }) => {
  const resp = await dumpAPI.createDump([]);
  const uid = JSON.parse(JSON.stringify(resp));
...
Scenario('PMM-T1835 - Download and Verify Dump Archive with QAN enabled @dump', async ({ dumpAPI, dumpPage, I }) => {
...
Scenario('PMM-T1835 - Download and Verify Dump Archive with QAN disabled @dump', async ({ dumpAPI, dumpPage, I }) => {
...
Scenario('PMM-T1835 - Check Dump Archives can be sent to Support in UI @dump', async ({ dumpAPI, dumpPage }) => {
  const resp = await dumpAPI.createDump([]);
  const uid = JSON.parse(JSON.stringify(resp));
...
Scenario('PMM-T1835 - Verify details of Dump based on Service Name @dump', async ({ dumpAPI, dumpPage }) => {
