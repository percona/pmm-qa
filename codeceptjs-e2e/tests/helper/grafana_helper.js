...
class Grafana extends Helper {
  ...
  // A PMM page left open keeps polling with the basic-auth header it was opened
  // with, so during a password change it spends Grafana's five-attempt lockout.
  // goto() directly: amOnPage awaits the Grafana iframe and would resolve
  // about:blank against the base URL.
  async parkPage() {
    const { Playwright } = this.helpers;
