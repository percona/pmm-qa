class MongodbBackupDetailsDashboard {
  ...
  // The panel renders "No data" until the pbm metric behind it is scraped. Poll
  // like the two helpers below, refreshing in the viewport so Grafana re-runs the
  // query.
  ...
  // waiting on it says nothing about Backup Sizes. Poll with the same scroll-and-
  // refresh the stat panels use: dashboardPage's wait helper never re-runs the
  // query, and this dashboard is opened without &refresh=.
